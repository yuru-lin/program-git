...
Public Class A_ECPriceList

    Dim DataAdapterDGV As SqlDataAdapter
    Dim ks1DataSetDGV As DataSet = New DataSet

    Private Sub A_ECPriceList_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
...
#Region "按鈕動作"
    ...
    Private Sub btnADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnADD.Click '新增
        A_ECPrice.Source = "ADD"
        A_ECPrice.MdiParent = MainForm
        A_ECPrice.Show()
        Me.Close()
    End Sub
#End Region
...
#Region "取得DataGridView資料"
    Private Sub GetDGV1Data()
        ...
        If ddlPriceType.SelectedIndex <> 0 Then
            strWhere += " AND (PriceType='" + ddlPriceType.SelectedItem + "') "
        End If
        ...
        If e.RowIndex <> -1 Then
            If DGV1.Columns(e.ColumnIndex).Name = "刪除" Then
                ...
                Try
                    ...
                    sql += "   WHERE ProCode='" + Convert.ToString(DGV1.Rows(e.RowIndex).Cells("存貨編號").Value) + "' AND PriceType='" + Convert.ToString(DGV1.Rows(e.RowIndex).Cells("PriceType").Value) + "' "
            ...
            If DGV1.Columns(e.ColumnIndex).Name = "送審" Then
                ...
                Try
                    sql += "   UPDATE KS_A_ECPrice  "
                    sql += "   SET Examine='1', Price=" + Convert.ToString(DGV1.Rows(e.RowIndex).Cells("價格").Value) + ", AlterDate=GETDATE(), AlterUser='" + Login.LogonUserName + "' "
                    sql += "   WHERE ProCode='" + Convert.ToString(DGV1.Rows(e.RowIndex).Cells("存貨編號").Value) + "' AND Examine='3' AND PriceType='" + Convert.ToString(DGV1.Rows(e.RowIndex).Cells("PriceType").Value) + "' "
