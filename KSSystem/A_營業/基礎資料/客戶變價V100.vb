...
Public Class 客戶變價V100   '20150618   980, 680
    ...
    Private Sub 單號存編2明細View() '單號存編2
        ...
        '原單價
        Dim 訂單明細原單價 As New DataGridViewTextBoxColumn()
        訂單明細原單價.DataPropertyName = "原單價"
        訂單明細原單價.HeaderText = "原單價"
        訂單明細原單價.Name = "原單價"
        訂單明細原單價.ReadOnly = True
        單號存編2.Columns.Add(訂單明細原單價)

        '狀態
        Dim 訂單明細狀態 As New DataGridViewTextBoxColumn()
        訂單明細狀態.DataPropertyName = "狀態"
        訂單明細狀態.HeaderText = "狀態"
        訂單明細狀態.Name = "狀態"
        '訂單明細狀態.Frozen = True
        訂單明細狀態.ReadOnly = True
        單號存編2.Columns.Add(訂單明細狀態)
    ...
    Private Sub T1變價單號_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles T1變價單號.CellClick
        ...
        La單號日期.Text = T1變價單號.CurrentRow.Cells("單號日期").Value
        La日期起始.Text = T1變價單號.CurrentRow.Cells("日期起始").Value
        La日期結束.Text = T1變價單號.CurrentRow.Cells("日期結束").Value
        La客戶群組.Text = T1變價單號.CurrentRow.Cells("客戶群組").Value
        La客戶編號.Text = T1變價單號.CurrentRow.Cells("客戶編號").Value
        La客戶名稱.Text = T1變價單號.CurrentRow.Cells("客戶名稱").Value
        La類別.Text = T1變價單號.CurrentRow.Cells("類別").Value
        La備註.Text = T1變價單號.CurrentRow.Cells("備註").Value
    ...
    Private Sub 單號存編2存編載入(ByVal Type As String)
        ...
        Select Case Type
            Case "新"
                SQLQuery = "    SELECT '' AS '選擇', '' AS '存編', '' AS '品名', '' AS '單價', '' AS '原單價', '' AS '狀態' "
            Case "原"
                SQLQuery = "  SELECT 'False' AS '選擇',[存編編號] AS '存編',[存編名稱] AS '品名',[單價],[單價] AS '原單價','正常' AS '狀態' "
                SQLQuery += "   FROM [KaiShingPlug].[dbo].[營_報價表身] "
    ...
    Private Sub Bu新增品項_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu新增品項.Click
    ...
    Private Sub Bu存檔_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu存檔.Click
        ...
        Dim xx As Integer = 0
        For ix As Integer = 0 To 單號存編2.RowCount - 1
            If 單號存編2.Rows(ix).Cells("單價").Value = 0 Then
                xx = xx + 1
            End If
        Next
        ...
        Select Case Bu存檔.Text
            ...
            Case "修改"
                'If 單號存編2.RowCount = 0 Then : Exit Sub : End If
                'Dim x As Integer = 0
                'For i As Integer = 0 To 單號存編2.RowCount - 1
                '    If 單號存編2.Rows(i).Cells("狀態").Value <> "正常" Then
                '        x = x + 1
                '    End If
                'Next
        ...
        TB單號.Text = ""
        品W1.Text = "" : 品W2.Text = "" : 品W3.Text = "" : 備註.Text = ""

        If TP切換限制 = "Y" Then : Me.TabControl1.TabPages.Add(TP客戶) : Me.TabControl1.TabPages.Remove(TP單號) : End If
    ...
    Private Sub 清空作業1()
        If 單號存編1.RowCount <> 0 Then : KsDataSetDGV.Tables("單號存編一").Clear() : dt1.Clear() : End If
        If 單號存編1.RowCount <> 0 Then : KsDataSetDGV.Tables("單號存編一s").Clear() : dt2.Clear() : End If
        If 單號存編2.RowCount <> 0 Then : KsDataSetDGV.Tables("單號存編二").Clear() : End If
    ...
    Private Sub 存檔2_變價資料(ByVal 單號 As String)
        Dim SQLADD1 As String = "" : Dim SQLADD2 As String = ""
        SQLADD1 = "   INSERT INTO [KaiShingPlug].[dbo].[營_報價表頭] "
        SQLADD1 += " ([單號],[單號日期],[日期起始],[日期結束],[客戶群組],[客戶編號],[客戶名稱],[類別],[備註],[啟用],[新增時間],[輸入者]) VALUES "
        SQLADD1 += " ('" & 單號 & "','" & Format(Now(), "yyyy-MM-dd") & "', "
        SQLADD1 += "  '" & Format(時間1.Value.Date, "yyyy-MM-dd") & "','" & Format(時間2.Value.Date, "yyyy-MM-dd") & "', "
        SQLADD1 += "  '','" & TB客編.Text & "','" & TB名稱.Text & "','" & 變價類別.Text & "','" & 備註.Text & "', "
        SQLADD1 += "  'N',GETDATE(),'') " & vbCrLf

        For i As Integer = 0 To 單號存編2.RowCount - 1
            SQLADD2 += "  INSERT INTO [KaiShingPlug].[dbo].[營_報價表身] ([單號],[存編群組],[存編編號],[存編名稱],[單價],[啟用],[新增時間],[備註]) VALUES "
            SQLADD2 += " ('" & 單號 & "','', "
            SQLADD2 += "  '" & 單號存編2.Rows(i).Cells("存編").Value & "', "
            SQLADD2 += "  '" & 單號存編2.Rows(i).Cells("品名").Value & "', "
            SQLADD2 += "  '" & 單號存編2.Rows(i).Cells("單價").Value & "', "
    ...
    Private Sub 修改2_變價資料(ByVal 單號 As String)
        Dim SQLADD表頭 As String = "" : Dim SQLADD0 As String = "" : Dim SQLADD1 As String = "" : Dim SQLADD2 As String = "" : Dim SQLADD3 As String = ""
        '表頭修正
        SQLADD表頭 = "  UPDATE [KaiShingPlug].[dbo].[營_報價表頭]  "
        SQLADD表頭 += "    SET [日期起始] = '" & Format(時間1.Value.Date, "yyyy-MM-dd") & "', "
        SQLADD表頭 += "        [日期結束] = '" & Format(時間2.Value.Date, "yyyy-MM-dd") & "', "
        SQLADD表頭 += "        [類別]     = '" & 變價類別.Text & "', "
        SQLADD表頭 += "        [備註]     = '" & 備註.Text & "', "
        SQLADD表頭 += "        [修改時間] = GETDATE() "
        SQLADD表頭 += "  WHERE [單號] = '" & 單號 & "' " & vbCrLf
        'SQLADD表頭 += "                [存編編號] = '" & 單號存編2.Rows(i).Cells("存編").Value & "' " & vbCrLf[修改時間]

        For i As Integer = 0 To 單號存編2.RowCount - 1
            If 單號存編2.Rows(i).Cells("狀態").Value = "新增" Then
                SQLADD0 += "  INSERT INTO [KaiShingPlug].[dbo].[營_報價表身] ([單號],[存編群組],[存編編號],[存編名稱],[單價],[啟用],[新增時間],[備註]) VALUES "
                SQLADD0 += " ('" & 單號 & "','', "
                SQLADD0 += "  '" & 單號存編2.Rows(i).Cells("存編").Value & "', "
                SQLADD0 += "  '" & 單號存編2.Rows(i).Cells("品名").Value & "', "
                SQLADD0 += "  '" & 單號存編2.Rows(i).Cells("單價").Value & "', "
                SQLADD0 += "  'Y',GETDATE(),'') " & vbCrLf
            End If

            If 單號存編2.Rows(i).Cells("狀態").Value = "修改" Then
                SQLADD1 += " UPDATE [KaiShingPlug].[dbo].[營_報價表身] SET [啟用] = 'U', [備註] = '修改單價' "
                SQLADD1 += "          WHERE [單號]     = '" & 單號 & "' AND "
                SQLADD1 += "                [存編編號] = '" & 單號存編2.Rows(i).Cells("存編").Value & "' " & vbCrLf
            ...
            End If
            If 單號存編2.Rows(i).Cells("狀態").Value = "刪除" Then
                SQLADD3 += " UPDATE [KaiShingPlug].[dbo].[營_報價表身] SET [啟用] = 'D', [備註] = '刪除' "
                SQLADD3 += "          WHERE [單號]     = '" & 單號 & "' AND "
                SQLADD3 += "                [存編編號] = '" & 單號存編2.Rows(i).Cells("存編").Value & "' " & vbCrLf
            End If
        ...
        Try
            SQLCmd.Connection = DBConn : SQLCmd.CommandText = SQLADD表頭 + SQLADD0 + SQLADD1 + SQLADD2 + SQLADD3 : SQLCmd.CommandTimeout = 100000
            SQLCmd.Parameters.Clear() : SQLCmd.ExecuteNonQuery()
            MsgBox("單號：" & 單號 & "新增完成")
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub T1修改_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles T1修改.Click
        ...
        '修改1_取變價表頭(TB單號.Text)
        Bu存檔.Text = "修改"
        時間1.Value = La日期起始.Text
        時間2.Value = La日期結束.Text
        變價類別.Text = La類別.Text
        備註.Text = La備註.Text
        單號存編2存編載入("原")
    End Sub

    Private Sub Bu1複製_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu1複製.Click
        If TB客編.Text = "" Or TB名稱.Text = "" Or TB單號.Text = "" Then : Exit Sub : End If
        If TP切換限制 = "Y" Then : Me.TabControl1.TabPages.Add(TP單號) : Me.TabControl1.TabPages.Remove(TP客戶) : End If
        Me.TabControl1.SelectedTab = Me.TP單號
        '修改1_取變價表頭(TB單號.Text)
    ...
    'Private Sub 單號存編2_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles 單號存編2.CellClick
    '    La單價.Text = 單號存編2.CurrentRow.Cells("單價").Value
    'End Sub

    Private Sub 單號存編2_CellValueChanged(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles 單號存編2.CellValueChanged
        'Dim 選擇 As String
        'If DBNull.Value.Equals(單號存編2.CurrentRow.Cells("選擇").Value) Then
        '    選擇 = "False"
        'Else
        '    選擇 = 單號存編2.CurrentRow.Cells("選擇").Value
        'End If
        'MsgBox(選擇)
        'If (選擇 <> "False" Or 選擇 <> "True") Then
        If 單號存編2.CurrentRow.Cells("原單價").Value = 單號存編2.CurrentRow.Cells("單價").Value Then
            Exit Sub
        End If

        If 單號存編2.CurrentRow.Cells("狀態").Value = "刪除" Then
            Exit Sub
        End If

        If 單號存編2.CurrentRow.Cells("狀態").Value = "正常" Then
            單號存編2.CurrentRow.Cells("狀態").Value = "修改"
        End If
        'End If
    ...
    Private Sub Bu刪除_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu刪除.Click
        Dim count As Integer = 單號存編2.Rows.Count
        For i As Integer = count - 1 To 0 Step -1
            Dim isSelected As Boolean '= Convert.ToBoolean(單號存編2.Rows(i).Cells("選擇").Value)
            If DBNull.Value.Equals(Convert.ToBoolean(單號存編2.Rows(i).Cells("選擇").Value)) Then
                isSelected = "False"
            Else
                isSelected = Convert.ToBoolean(單號存編2.Rows(i).Cells("選擇").Value)
            End If
            If isSelected Then
                If 單號存編2.Rows(i).Cells("狀態").Value = "正常" Or 單號存編2.Rows(i).Cells("狀態").Value = "修改" Then
                    單號存編2.Rows(i).Cells("狀態").Value = "刪除"
                End If
                If 單號存編2.CurrentRow.Cells("狀態").Value = "新增" Then
                    單號存編2.Rows.Remove(單號存編2.Rows(i))
                End If
            End If
        Next
    ...
    Private Sub Bu2全選_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu2全選.Click
        Dim count As Integer = 單號存編2.Rows.Count
        For i As Integer = count - 1 To 0 Step -1
            單號存編2.Rows(i).Cells("選擇").Value = True
           
        Next

    End Sub

    Private Sub T1刪除_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles T1刪除.Click
    ...
    Private Sub T1刪除_變價資料(ByVal 單號 As String)
        Dim SQLADD表頭 As String = ""
        '表頭修正
        SQLADD表頭 = "  UPDATE [KaiShingPlug].[dbo].[營_報價表頭]  "
        SQLADD表頭 += "    SET [啟用] = 'D', "
        SQLADD表頭 += "        [修改時間] = GETDATE() "
        ...
        Try
            SQLCmd.Connection = DBConn : SQLCmd.CommandText = SQLADD表頭 : SQLCmd.CommandTimeout = 100000
            SQLCmd.Parameters.Clear() : SQLCmd.ExecuteNonQuery()
            MsgBox("單號：" & 單號 & "刪除完成")
        Catch ex As Exception
            MsgBox(ex.Message)
