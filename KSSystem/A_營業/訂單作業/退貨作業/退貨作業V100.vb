...
Public Class 退貨作業V100
    ...
    Private Sub 退貨作業_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Load初始化()
        ...
        Lt處理方式.Text = "" : Lt備註1.Text = "" : Lt備註2.Text = "" : Lt備註3.Text = ""
        Lt輸入者.Text = ""
    ...
    Private Sub T1退貨單號載入()
        ...
        SQLQuery += "          END, '') AS '備註1',[備註] AS '備註2',ISNULL([說明],'') AS '備註3', "
        SQLQuery += "          [啟用],[輸入者],[來源],[類別],[稅額],[總計],[列印否],[列印次數] "
    ...
    Private Sub T1退貨單號_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles T1退貨單號.CellClick
        ...
        Lt備註3.Text = T1退貨單號.CurrentRow.Cells("備註3").Value
    ...
    Private Sub Bu1列印作業()
        ...
        If 顯示資料.Tables("T1退貨明細").Rows.Count > 0 Then
            ...
            newRpt.strPara(11) = Lt退貨原因.Text & vbCrLf & Lt備註2.Text & vbCrLf & Lt備註3.Text
            Dim strPara12 As String = ""
            'strPara12 = "稅額 " & CStr(StrDup(10 - Len(CStr(Lt稅額.Text)), " ")) & CStr(Lt稅額.Text) & " NTD" & vbCrLf
            'strPara12 += "總計 " & CStr(StrDup(10 - Len(CStr(Lt總計.Text)), " ")) & CStr(Lt總計.Text) & " NTD"
    ...
    Private Sub Bu2選單_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu2選單.Click
        ...
        '    MsgBox("錯誤已有退貨單號：" + CStr(查詢是否已有退貨單號())) : Exit Sub
        'End If
    ...
    Private Sub T3退貨明細稅額()
        Dim 總計1 As Single = 0 : Dim 稅額1 As Single = 0 : Dim 稅額2 As String = "N"
        For i As Integer = 0 To T3退貨明細.RowCount - 1
            If T3退貨明細.Rows(i).Cells("退貨數量").Value <> 0 Then
                T3退貨明細.Rows(i).Cells("金額").Value = CDbl(T3退貨明細.Rows(i).Cells("單價").Value) * CDbl(T3退貨明細.Rows(i).Cells("退貨數量").Value)
    ...
    Private Sub T3存檔(ByVal 作業 As String, ByVal 單號 As String)
        ...
        Select Case 作業
            Case "新增"
                ...
                SQLADD1 += "   [聯絡人],[電話],[傳真],[地址],[處理方式],[退貨代碼],[司機代碼],[備註],[說明], "
                ...
                SQLADD1 += "  '" & Replace(Lt聯絡人.Text, "'", "") & "','" & Replace(Lt電話.Text, "'", "") & "','" & Lt傳真.Text & "','" & Lt地址.Text & "','" & Lt處理方式.Text & "','" & Cb3退貨原因.SelectedValue.ToString & "','" & Cb3運輸司機.SelectedValue.ToString & "', "
                SQLADD1 += "  '" & Lt備註2.Text & "','" & Lt備註3.Text & "','Y',GETDATE(),'" & Lt輸入者.Text & "','" & Lt來源.Text & "',"
        ...
        For i As Integer = 0 To T3退貨明細.RowCount - 1
            If T3退貨明細.Rows(i).Cells("退貨數量").Value <> 0 Then
                ...
                SQLADD2 += " ('" & 單號 & "', "
                SQLADD2 += "  CAST('" & T3退貨明細.Rows(i).Cells("存貨編號").Value & "' AS VARCHAR(20)), "
        ...
        Dim SQLADD3 As String = "" : Dim SQLADD4 As String = ""

        '修正_先作廢
        SQLADD3 = "  UPDATE [KaiShingPlug].[dbo].[營_退折明細] "
        SQLADD3 += "    SET [啟用]     = 'N', "
        SQLADD3 += "        [修改時間] = GETDATE() "
        SQLADD3 += "  WHERE [單號] = '" & 單號 & "' AND 啟用 = 'Y' " & vbCrLf

        For i As Integer = 0 To T4輸入資料.RowCount - 1
            SQLADD4 += " INSERT INTO [KaiShingPlug].[dbo].[營_退折明細] "
            SQLADD4 += " ([單號],[存貨編號],[存編名稱],[退貨數量],[營_類別],[營_退貨原因],[營_補充說明],[列號],[啟用],[新增時間]) VALUES "
            SQLADD4 += " ('" & 單號 & "', "
            SQLADD4 += "  CAST('" & T4輸入資料.Rows(i).Cells("存貨編號").Value & "' AS VARCHAR(20)), "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("存編名稱").Value & "', "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("退貨數量").Value & "', "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("類別代碼").Value & "', "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("原因代碼").Value & "', "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("補充說明").Value & "', "
            SQLADD4 += "  '" & T4輸入資料.Rows(i).Cells("列號").Value & "'    , "
            SQLADD4 += "  'Y',GETDATE() )"
        Next
        ...
        'MsgBox(SQLADD表頭 + SQLADD表身)
        'MsgBox(SQLADD1 + SQLADD2)
        'MsgBox(SQLUP)
        'MsgBox(SQLADD3 + SQLADD4)

        Dim DBConn As SqlConnection = Login.DBConn
        Dim SQLCmd As SqlCommand = New SqlCommand

        Try
            SQLCmd.Connection = DBConn : SQLCmd.CommandText = CStr(SQLADD表頭 + SQLADD表身 + SQLADD1 + SQLADD2 + SQLUP + SQLADD3 + SQLADD4) : SQLCmd.CommandTimeout = 100000
            SQLCmd.Parameters.Clear() : SQLCmd.ExecuteNonQuery()
            Select Case 作業
                Case "新增"
                    MsgBox("單號：" & 單號 & "新增完成")
    ...
    Private Sub T3退貨明細載入()
        If T3退貨明細.RowCount > 0 Then : 顯示資料.Tables("退貨明細_退").Clear() : End If '清除資料
        Dim SQLQuery As String = ""
        SQLQuery = "    SELECT [存貨編號],[存編名稱],CAST([退貨數量] AS FLOAT(53)) AS '退貨數量',[數量],[出貨重量],[單位],[數量2],[小單位],[單價],[計價],[金額],[倉別],[稅碼],[列號],[備註] "
        SQLQuery += "     FROM [KaiShingPlug].[dbo].[營_退折表身] "
        SQLQuery += "    WHERE [單號] = '" & TB單號.Text & "' AND [啟用] = 'Y' "
        SQLQuery += " ORDER BY [列號] "
    ...
    Private Sub T4退貨數量載入()
        ...
        SQLQuery += "     ) T3 ON T1.[營_退貨原因] = T3.[代碼]    "
    ...
    Private Sub Bu4輸入_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu4輸入.Click
        If CStr(Lt4數量.Text) = "" Or CDbl(Lt4數量.Text) <> 0 Then
            Dim Row As DataRow
    ...
    Private Sub Bu4移除_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu4移除.Click
        If Lt4存編.Text = T4輸入資料.CurrentRow.Cells("存貨編號").Value Then
            Dim i As Integer = 0
            i = T4輸入資料.CurrentCell.RowIndex()   '
            Dim oAns As Integer
            'oAns = MsgBox("是否確定刪除選定資料  " & (T4輸入資料.CurrentCell.RowIndex() + 1), MsgBoxStyle.YesNo, "刪除選定資料")
            oAns = MsgBox("是否確定刪除選定資料  列" & (T4輸入資料.CurrentCell.RowIndex() + 1) & vbCrLf & T4輸入資料.CurrentRow.Cells("存貨編號").Value, MsgBoxStyle.YesNo, "刪除選定資料")
            If oAns = MsgBoxResult.Yes Then  'Yes執行區
    ...
    Private Sub T4退貨數量合計更新()
        ...
        For i As Integer = count - 1 To 0 Step -1
            ...
            End If
            Tb3說明.Text = ""
            Tb3說明.Text += T4輸入資料.Rows(i).Cells("補充說明").Value
        Next
    ...
    Private Sub Bu4存檔_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bu4存檔.Click
        ...
        'T3退貨明細.CurrentRow.Cells("退貨數量").Value = CDbl(T3退貨明細.CurrentRow.Cells("單價").Value) * CDbl(T3退貨明細.CurrentRow.Cells("退貨數量").Value)
        T3退貨明細稅額()
    ...
    Private Sub T3說明更新()
        ...
        Dim count As Integer = T4輸入資料.Rows.Count
        For i As Integer = count - 1 To 0 Step -1
            Tb3說明.Text += T4輸入資料.Rows(i).Cells("補充說明").Value & vbCrLf
        Next
