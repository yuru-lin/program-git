...
#Region "按鈕動作"
    ...
    Private Sub InsertData()
        ...
        SQLCmd.CommandText = String.Format(" INSERT INTO [KS_Z_StockApply] ([ID],[DepartType] " & _
         ",[IndexID],[Examine],[MEMO],[AddUser]) VALUES ('{0}','{1}','{2}','1','{3}','{4}') " _
         , ID + SerNum _
...
#Region "取得DataGridView資料"
    ...
    Private Sub GetDGV1Data_Edit()
        ...
        Using TransactionMonitor As New System.Transactions.TransactionScope
            ...
            sql = "    Select ZS.ID, ZS.DepartType, ZS.IndexID, ZS.MEMO AS '用途備註','' AS MEMO, ProCode, ProName, Num, ZSD.Fldvalue, T0.Descr as 'FldName',  "
            sql += "    CASE [Type]  "
            sql += "   	    WHEN '1' THEN '電宰品' "
            sql += "    	WHEN '2' THEN '加工品' "
            sql += "    	WHEN '3' THEN '其他品' "
