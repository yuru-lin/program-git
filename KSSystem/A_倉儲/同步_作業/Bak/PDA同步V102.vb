...
Public Class PDA同步V102
    ...
    Private Sub PDA同步V102_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub 讀取條碼入庫()
        ...
        Dim SQLQuery As String = ""
        SQLQuery = "     IF (OBJECT_ID('tempdb.." & SyncPDA1 & "') IS NOT NULL)  DROP TABLE " & SyncPDA1 & " "
        SQLQuery += "    SELECT * INTO " & SyncPDA1 & " FROM [KaiShingPlug].[dbo].[PDA_InData] WHERE LEFT([PDA02],2) IN ('11','12') AND [PDA06] = '2' "
    ...
    Private Sub 讀取條碼出庫()
        ...
        SQLQuery += "     FROM [KaiShingPlug].[dbo].[PDA_InData] TX WHERE TX.[PDA06] = '2' AND LEFT(TX.[PDA02],2) IN ('21','22','25','31','32','41','42') "
        SQLQuery += "   IF (OBJECT_ID('tempdb.." & SyncPDA2 & "') IS NOT NULL) DROP TABLE " & SyncPDA2 & " "
        SQLQuery += "   SELECT * INTO " & SyncPDA2 & " FROM ( "
        ...
        For Each column As DataGridViewColumn In DGV1.Columns
            column.Visible = True
            Select Case column.Name
                Case "作業別類" : column.HeaderText = "作業別類" : column.DisplayIndex = 0 : column.Frozen = True
                Case "條碼" : column.HeaderText = "條碼" : column.DisplayIndex = 1 : column.Frozen = True
                Case "單號" : column.HeaderText = "單號" : column.DisplayIndex = 2
                Case "存編" : column.HeaderText = "存編" : column.DisplayIndex = 3
                Case "品名" : column.HeaderText = "品名" : column.DisplayIndex = 4
                Case "數量" : column.HeaderText = "數量" : column.DisplayIndex = 5
                Case "庫存" : column.HeaderText = "庫存" : column.DisplayIndex = 6
                Case "重量" : column.HeaderText = "重量" : column.DisplayIndex = 7
                Case "儲位" : column.HeaderText = "儲位" : column.DisplayIndex = 8
                Case "製造單號" : column.HeaderText = "製造單號" : column.DisplayIndex = 9
                Case "生產日期" : column.HeaderText = "生產日期" : column.DisplayIndex = 10
                ...
                Case "數量異常" : column.HeaderText = "數量異常" : column.DisplayIndex = 14
                Case Else
                    column.Visible = False
            End Select
        Next
    ...
    Private Sub DGV1_入庫檢查資料()
        ...
        La入庫.Text = Cnt0 & "筆 已入庫"
        La出庫.Text = Cnt1 & "筆 已出庫"
        La作廢.Text = Cnt2 & "筆 已作廢"
        La異常.Text = Cnt3 & "筆 查無條碼"
