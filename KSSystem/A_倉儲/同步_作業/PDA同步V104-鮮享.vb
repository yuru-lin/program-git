    ...
    Private Sub 異常排除條碼()
        ...
        For Each oRow As DataGridViewRow In DGV1.Rows
            ...
            If isSelected = True Then
                ...
                If oRow.Cells("管理").Value Like "[B,S]" Then

                    If oRow.Cells("重複").Value = "重複" And oRow.Cells("入庫否").Value = "已入庫" Then
                    ...
                    If CB刪除.Checked = True Then
                        If oRow.Cells("入庫否").Value = "未入庫" Then
                            If 已轉出1 = "" Then : 已轉出1 = oRow.Cells("ID").Value : Else : 已轉出1 += "," & oRow.Cells("ID").Value : End If
                        End If
        ...
        If SQLQuery = "" Then : MsgBox("無異常可移除") : Exit Sub : End If
        'MsgBox(SQLQuery)
