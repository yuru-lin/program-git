...
Public Class 出庫作業V002   '20181126
    ...
    Private Sub 同步SAP1()  '出貨作業別類 = ["領料","領用"]
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                OIGEAdd.Comments = 草稿T2.Text & vbCrLf & 備註T2.Text & vbCrLf & 說明T2.Text           '備註
            Case Else
        ...
        For j As Integer = 0 To T2DGV1.RowCount - 1
            ...
            If 作業編號 = "3" Then
                OIGEAdd.Lines.WarehouseCode = T2DGV1.Rows(j).Cells("庫位").Value
            Else
                ...
                Select Case 作業編號
                    Case "3"
                        If (Trim(T2DGV2.Rows(i).Cells("存編").Value) = Trim(T2DGV1.Rows(j).Cells("存編").Value)) And _
                           (Trim(T2DGV2.Rows(i).Cells("庫位").Value) = Trim(T2DGV1.Rows(j).Cells("庫位").Value)) Then
                            'Select Case 作業編號
                            '    Case "0"    '0.排程領料
                            '    Case "1"    '1.單據領料
                            '    Case "2"    '2.銷售出庫
                            '    Case "3"    '3.存貨領用
                            '        OIGEAdd.Lines.WarehouseCode = T2DGV2.Rows(i).Cells("庫位").Value
                            '    Case "4"    '0.採購入庫
                            '    Case "5"    '4.寄庫出庫
                            'End Select
    ...
    Private Sub 同步SAP2()  '出貨作業別類 = ["銷售"]
        ...
        oDelv.UserFields.Fields.Item("U_P5_Comments").Value = 備註T2倉.Text
        ...
        For j As Integer = 0 To T2DGV1.RowCount - 1
            ...
            If 預設庫位YN.Checked = True Then
                oDelv.Lines.WarehouseCode = TB預設庫位.Text
            End If
            ...
            '[VatGroup]稅別 O5%
            Select Case T2DGV1.Rows(j).Cells("管理").Value
                Case "B", "F"
                    oDelv.Lines.DiscountPercent = 0.0
                    oDelv.Lines.PriceAfterVAT = T2DGV1.Rows(j).Cells("含稅單價").FormattedValue
            End Select

            'oDelv.Lines.LineTotal = 
            'Select Case T2DGV2.Rows(j).Cells("管理").Value
            '    Case "B"

            Select Case T2DGV1.Rows(j).Cells("計價單位").Value
                Case "0"
                    oDelv.Lines.UnitPrice = T2DGV1.Rows(j).Cells("銷售金額").Value
                    'oDelv.Lines.UnitPrice = dgvSourceList.Rows(i).Cells("U_P8").Value
                Case "1"
                    oDelv.Lines.UnitPrice = (T2DGV1.Rows(j).Cells("銷售金額").Value * T2DGV1.Rows(j).Cells("出庫重量").Value) / T2DGV1.Rows(j).Cells("出庫數量").Value
                    'oDelv.Lines.UnitPrice = (dgvSourceList.Rows(i).Cells("U_P8").Value * p2Value) / dgvSourceList.Rows(i).Cells("Quantity").Value
                Case "2"
                    oDelv.Lines.UnitPrice = T2DGV1.Rows(j).Cells("銷售金額").Value * T2DGV1.Rows(j).Cells("單位2數量").Value / T2DGV1.Rows(j).Cells("出庫數量").Value
            ...
            'If T2DGV1.Rows(j).Cells("計價單位").Value = 2 Then
            '    oDelv.Lines.UnitPrice = T2DGV1.Rows(j).Cells("銷售金額").Value * T2DGV1.Rows(j).Cells("單位2數量").Value / T2DGV1.Rows(j).Cells("出庫數量").Value
            '    'oDelv.Lines.Price = T2DGV1.Rows(j).Cells("實際金額").Value * T2DGV1.Rows(j).Cells("單位2數量").Value / T2DGV1.Rows(j).Cells("出庫數量").Value
            '    'oDelv.Lines.UnitPrice = (dgvSourceList.Rows(i).Cells("U_P8").Value * dgvSourceList.Rows(i).Cells("U_P4").Value) / dgvSourceList.Rows(i).Cells("Quantity").Value
            'Else
            '    oDelv.Lines.UnitPrice = T2DGV1.Rows(j).Cells("實際金額").FormattedValue
            'End If
            '    Case Else
            'oDelv.Lines.UnitPrice = T2DGV1.Rows(j).Cells("實際金額").FormattedValue
            'End Select
    ...
    Private Sub 查詢單據_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 查詢單據T1.Click
        ...
        Select Case 作業選項T1.Text
            Case "0.排程領料" : 作業T1.Text = "排程領料" : 作業編號 = "0" : 出貨別類 = "發貨" : 作業別號 = " =   '24' "       '排程  24,25
        ...
        End Select
        If 作業編號 = "3" Then : T1CB實際庫位.Visible = True : Else : T1CB實際庫位.Visible = False : End If
        T1CB實際庫位.Checked = False
        查詢單據()

    End Sub

    Private Sub 查詢單據()
        If T1DGV1.RowCount > 0 Then : ks1DataSetDGV.Tables("T1DGV1").Clear() : T1DGV1.DataSource = Nothing : End If '清除DGV1資料

        Dim SQLADD As String = ""

        Select Case 作業編號
            Case "0"    '0.排程領料
                SQLADD = " EXEC [dbo].[預_分時領料明細01v] 'AD',N'',N'',NULL,NULL "
            Case "1"    '1.單據領料
                SQLADD = "     SELECT CAST(T0.[DocEntry] AS VARCHAR(50)) AS '草稿', CONVERT(VARCHAR(10), T0.[DocDueDate], 120) AS '日期', T0.[U_CK01] '代碼', ISNULL(T0.[U_CK02],'') AS '製單', ISNULL(T0.[Comments],'') AS '備註', T0.[JrnlMemo] '日記', '' AS '提單' "
                SQLADD += "      FROM [ODRF] T0 "
                SQLADD += "     WHERE T0.[ObjType] = '60' AND T0.[U_CK01] = '5' AND T0.[DocStatus] = 'O' ORDER BY T0.[DocEntry] DESC "
            Case "2"    '2.銷售出庫
                SQLADD = "    SELECT DISTINCT CAST(T0.[DocEntry] AS VARCHAR(50)) AS '草稿', CONVERT(VARCHAR(10), T0.[DocDueDate], 120) AS '日期', T0.[U_CK01] '代碼', ISNULL(T0.[U_CK02],'') AS '製單', "
                'SQLADD += "          T0.[Comments] AS '備註', ISNULL(T0.[U_P5_Comments],"") AS '備註', T0.[JrnlMemo] '日記', "
                SQLADD += "          ISNULL(T0.[U_P5_Comments],'') AS '備註', T0.[Comments] AS '備註A', T0.[JrnlMemo] '日記', "
                SQLADD += "          T0.[U_Dnum] AS '提單', T0.[CardCode] AS '客編', T0.[CardName] AS '客戶', T0.[U_Dlst] AS '出貨單回簽否', T0.[U_Cnum] AS '契養批號', T0.[U_TpMoney] AS '運費', "
                SQLADD += "          T0.[U_CarDr1] AS '司機1', T0.[U_CarDr2] AS '司機2', T0.[U_CarDr3] AS '司機3', T0.[U_attachment] AS '附件', T0.[U_updnum] AS '更新空籃數', T0.[U_N1] AS '前期空籃', T0.[U_N2] AS '本期空籃', T0.[U_N3] AS '本期收回', "
            ...
            Case "3"    '3.存貨領用
                ...
                SQLADD += "     FROM [KS_Z_StockApply] K INNER JOIN [KS_Z_StockApply_Detail] KD ON K.[ID]      = KD.[ID] "
            ...
            Case "4"    '0.採購入庫
                SQLADD = "     SELECT DISTINCT CAST(T0.[DocEntry] AS VARCHAR(50)) AS '草稿', CONVERT(VARCHAR(10), T0.[DocDueDate], 120) AS '日期', T0.[U_CK01] '代碼', T0.[Comments] AS '備註', T0.[JrnlMemo] '日記', "
            ...
            Case "5"    '4.寄庫出庫
                ...
                SQLADD = "    SELECT DISTINCT CAST(T0.[DocEntry] AS VARCHAR(50)) AS '草稿', CONVERT(VARCHAR(10), T0.[DocDueDate], 120) AS '日期', T0.[U_CK01] '代碼', ISNULL(T0.[U_CK02],'') AS '製單', T0.[Comments] AS '備註','' AS '備註A', T0.[JrnlMemo] '日記', "
                SQLADD += "          T0.[U_Dnum] AS '提單', T0.[CardCode] AS '客編', T0.[CardName] AS '客戶', T0.[U_Dlst] AS '出貨單回簽否', T0.[U_Cnum] AS '契養批號', T0.[U_TpMoney] AS '運費', "
                SQLADD += "          T0.[U_CarDr1] AS '司機1', T0.[U_CarDr2] AS '司機2', T0.[U_CarDr2] AS '司機3', T0.[U_attachment] AS '附件', T0.[U_updnum] AS '更新空籃數', T0.[U_N1] AS '前期空籃', T0.[U_N2] AS '本期空籃', T0.[U_N3] AS '本期收回', "
                SQLADD += "          T0.[U_COSCO] AS '好市多訂單編號' "
                SQLADD += "     FROM [ODRF] T0 LEFT JOIN [DRF1] T1 ON T0.[DocEntry] = T1.[DocEntry] "
            ...
            Select Case 作業編號
                Case "0"    '0.排程領料
                    Select Case column.Name
                        ...
                        Case "樓層" : column.HeaderText = "樓層" : column.DisplayIndex = 3
                        Case "排程時間" : column.HeaderText = "排程時間" : column.DisplayIndex = 4
                        Case "製單" : column.HeaderText = "製單" : column.DisplayIndex = 5
                        Case Else
                ...
                Case "2", "5"    '2.銷售出庫, 4.寄庫出庫
                    Select Case column.Name
                        ...
                        Case "備註A" : column.HeaderText = "備註A" : column.DisplayIndex = 5
                        Case Else
                            column.Visible = False
                    End Select
                Case "3"    '3.存貨領用
    ...
    Private Sub 草稿比對T1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 草稿比對T1.Click
        ...
        Select Case 作業編號
            ...
            Case "2", "5"    '2.銷售出庫, 4.寄庫出庫
                ...
                代碼T2.Text = T1DGV1.CurrentRow.Cells("代碼").Value
                製單T2.Text = T1DGV1.CurrentRow.Cells("製單").Value
                備註T2.Text = T1DGV1.CurrentRow.Cells("備註A").Value '& vbCrLf & T1DGV1.CurrentRow.Cells("備註").Value
                備註T2倉.Text = T1DGV1.CurrentRow.Cells("備註").Value
                日記T2.Text = T1DGV1.CurrentRow.Cells("日記").Value
        ...
        If 強制出庫.Checked = False And G1 <> 0 Then
            發貨作業T2.Visible = False
            MsgBox("出庫品項有異無法作業，請查詢後在作業!!!")
        End If

        If 強制出庫.Checked = True Then
            If Microsoft.VisualBasic.Left(未入T2.Text, 1) = "0" And _
               Microsoft.VisualBasic.Left(已出T2.Text, 1) = "0" And _
               Microsoft.VisualBasic.Left(重覆T2.Text, 1) = "0" And _
               Microsoft.VisualBasic.Left(數量T2.Text, 1) = "0" And _
               Microsoft.VisualBasic.Left(品項T2.Text, 1) <> "0" Then
                發貨作業T2.Visible = True
            End If
        End If


        If 作業T1.Text = "單據領料" And Mid(Replace(日期T2.Text, "-", ""), 3, 6) <> Mid(製單T2.Text, 2, 6) Then
            'MsgBox(Mid(Replace(日期T2.Text, "-", ""), 3, 6) & "    " & Mid(製單T2.Text, 2, 6))
            La差異.Visible = True
            MsgBox("日期和製單有差異", 16)
        End If
    ...
    Private Sub T2發貨作業()
        If Cnt0 = 0 And Cnt1 = 0 And Cnt2 = 0 And Cnt3 = 0 And Cnt4 = 0 And Cnt5 = 0 Then
            '查看異常T2.Visible = False
            發貨作業T2.Visible = True
            放棄作業T2.Location = New Point(710, 546)
            申請作業T2.Visible = False
            La2申請.Visible = False
            If Login.LoginType = 2 Then : 發貨作業T2.Enabled = False : Else : 發貨作業T2.Enabled = True : End If '未SAP權限登入無法執行入庫
        Else
            'If UCase(GetHostName()) = "KS-I11" Or UCase(GetHostName()) = "MIS-03" Or UCase(GetHostName()) = "MIS-06" Or UCase(GetHostName()) = "KS-NB-T1" Then
            ...
            'End If
            發貨作業T2.Visible = False
            If 作業編號 = "0" And T2DGV2.RowCount > 0 Then
                If La2申請.Text = "." Then
                    申請作業T2.Visible = True
                    La2申請.Visible = False
                Else
                    ...
                    La2申請.Visible = True
                End If
                放棄作業T2.Location = New Point(770, 546)
            End If

        End If
    End Sub

    Private Sub 申請作業T2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 申請作業T2.Click
        T1分時領料存檔()
        放棄作業T2.PerformClick()
    End Sub
    ...
    Private Sub T1分時領料存檔()
        ...
        SQLADD1 += " INSERT INTO [KaiShingPlug].[dbo].[倉_分時項目確認] "
        SQLADD1 += " ([日期],[分時領料單],[項目同意],[新增時間]) VALUES "
        'SQLADD1 += " ('" & Format(Now(), "yyyy-MM-dd") & "' , "
        SQLADD1 += " ('" & 日期T2.Text & "' , "
        SQLADD1 += "  '" & 草稿T2.Text & "' , "
        SQLADD1 += "  'N', GETDATE() ) "
    ...
    Private Sub T2查詢作業()
        If 作業編號 = "3" Then
            If T1CB實際庫位.Checked = False Then : 存領庫位 = "N" : Else : 存領庫位 = "Y" : End If
        End If
        ...
        T2DGV3.DataSource = Nothing
        Select Case 作業編號
            'Case "0"    '0.排程領料
            Case "0", "1", "2", "3", "4", "5"   '1.單據領料, 2.銷售出庫, 0.採購入庫, 4.寄庫出庫
                T2查詢單據非單據品項()
    ...
    Private Sub T2DGV1異常檢查()
        ...
        Select Case 作業編號
            'Case "0"     '0.排程領料
            '    品項T2.Text = "0 筆非單據品項"
            Case "0", "1", "2", "5"   '1.單據領料, 2.銷售出庫, 5.寄庫出庫
    ...
    Private Sub T2DGV2異常檢查()
        ...
        For i As Integer = 0 To T2DGV2.RowCount - 1   '以Blue底顯示
            If T2DGV2.Rows(i).Cells("G1").Value <> "" And T2DGV2.Rows(i).Cells("G_OK").Value = "" Then
                'T2DGV2.Rows(i).DefaultCellStyle.BackColor = Color.Blue
                G1 += 0 + 1
            End If : Next
    ...
    Private Sub T2查詢單據暫存作業()
        ...
        Select Case 入庫出貨
            Case "出貨"
                ...
                SQLQuery += "       SELECT 'S' AS '管理', T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], ISNULL(T2.[Quantity],9999) AS 'Quantity', T1.[U_M1], T1.[U_M8], T1.[MnfDate], T1.[ExpDate], T2.[WhsCode] "
                ...
                SQLQuery += "                        INNER JOIN ("
                SQLQuery += "                                   SELECT T1.[ItemCode],T1.[DistNumber],T1.[SysNumber],MAX(T2.[AbsEntry]) AS 'AbsEntry' "
                SQLQuery += "                                     FROM [OSRN] T1 INNER JOIN [OSRQ] T2 ON T1.[ItemCode]   = T2.[ItemCode] AND T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry] "
                SQLQuery += "                                                    INNER JOIN " & Outtmp2 & " T4 ON T1.[DistNumber] = T4.[DistNumber] "
                SQLQuery += "                                 GROUP BY T1.[ItemCode],T1.[DistNumber],T1.[SysNumber] "
                SQLQuery += "                                   ) T5 ON T1.[ItemCode]   = T5.[ItemCode] AND T1.[DistNumber] = T5.[DistNumber] AND T2.[AbsEntry] = T5.[AbsEntry] "
                SQLQuery += "                        INNER JOIN [OITM]          T3 ON T1.[ItemCode]   = T3.[ItemCode] "
                SQLQuery += "                        INNER JOIN " & Outtmp2 & " T4 ON T1.[DistNumber] = T4.[DistNumber] "
                SQLQuery += " UNION ALL"
                SQLQuery += "       SELECT 'B' AS '管理', T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], ISNULL(T2.[Quantity],9999) AS 'Quantity', T1.[U_M1], T1.[U_M8], T1.[MnfDate], T1.[ExpDate], T2.[WhsCode] "
            ...
            Case "入庫"
                ...
                SQLQuery += "  IF (OBJECT_ID('tempdb.." & Outtmp3 & "') IS NOT NULL)  DROP TABLE " & Outtmp3 & " "
                SQLQuery += " SELECT * INTO " & Outtmp3 & " FROM ( "
                SQLQuery += "       SELECT 'S' AS '管理', T0.[FI107K] AS 'ItemCode', T0.[FI108] AS 'ItemName', T0.[U_M2], '1' AS 'Quantity', T0.[FI118] AS 'U_M1', '' AS 'U_M8', T0.[FI110] AS 'MnfDate', T0.[FI111] AS 'ExpDate' "
    ...
    Private Sub T2查詢單據條碼()
        ...
        Select Case 實際編號
            'Case "0"    '0.排程領料
            Case "0", "1", "2", "3", "5"    '1.單據領料, 2.銷售出庫, 3.存貨領用, 5.寄庫出庫
                ...
                SQLQuery += "           T1.[WhsCode] AS '庫位', T1.[U_M2] AS '儲位', T1.[MnfDate] AS '製造日期', T1.[ExpDate] AS '保存日期', T1.[U_M8] AS '製造單號', "
                ...
                Select Case 實際編號 '銷售別類
                    Case "0", "1", "3", "5"    '1.單據領料, 3.存貨領用, 4.寄庫出庫
                        SQLQuery += "   '銷售' AS '銷售', "
                    Case "2"         '2.銷售出庫
                ...
                SQLQuery += "  GROUP BY T1.[ItemCode], T1.[ItemName], T0.[Quantity], T1.[Quantity], T1.[MnfDate], T1.[ExpDate], T0.[DistNumber], T1.[U_M1], T1.[WhsCode], T1.[U_M2], T1.[U_M8], T4.[M_NO], T5.[M_NO], T1.[管理], T0.[Gift], T2.[狀態], T0.[ID] "
            ...
            Case "4"        '0.採購入庫
                ...
                SQLQuery += "  GROUP BY T1.[ItemCode], T1.[ItemName], T0.[Quantity], T1.[Quantity], T1.[MnfDate], T1.[ExpDate], T0.[DistNumber], T1.[U_M1], T1.[WhsCode], T1.[U_M2], T1.[U_M8], T1.[管理], T0.[Gift], T2.[狀態], T0.[ID] "
                SQLQuery += "  ORDER BY COUNT(T0.[DistNumber]) DESC ,T0.[DistNumber] "
        ...
        Try
            ...
            Select Case 作業查詢
                ...
                Case "草稿比對"
                    DataAdapterDGV.Fill(ks1DataSetDGV, "T2DGV2") : T2DGV2.DataSource = ks1DataSetDGV.Tables("T2DGV2")
            End Select
    ...
    Private Sub T2查詢單據條碼_Play()
        ...
        For Each column As DataGridViewColumn In T2DGV2.Columns
            ...
            Select Case column.Name
                ...
                Case "保存日期" : column.HeaderText = "保存日期" : column.DisplayIndex = 5
                Case "庫存" : column.HeaderText = "庫存" : column.DisplayIndex = 6
                Case "重量" : column.HeaderText = "重量" : column.DisplayIndex = 7
                Case "庫位" : column.HeaderText = "庫位" : column.DisplayIndex = 8
                Case "儲位" : column.HeaderText = "儲位" : column.DisplayIndex = 9
                Case "製造單號" : column.HeaderText = "製造單號" : column.DisplayIndex = 10
                Case "狀態" : column.HeaderText = "狀態" : column.DisplayIndex = 11
                Case "重覆" : column.HeaderText = "重覆" : column.DisplayIndex = 12
                Case "銷售" : column.HeaderText = "銷售" : column.DisplayIndex = 13
                Case "G1" : column.HeaderText = "G1" : column.DisplayIndex = 14
                Case "G_OK" : column.HeaderText = "G_OK" : column.DisplayIndex = 15
                Case "管理" : column.HeaderText = "管理" : column.DisplayIndex = 16
                Case "ID" : column.HeaderText = "ID" : column.DisplayIndex = 17
                Case Else
                    column.Visible = False
            End Select
        Next
    ...
    Private Sub T2查詢單據明細()    '品項資料
        If T2DGV1.RowCount > 0 Then : ks1DataSetDGV.Tables("T2DGV1").Reset() : T2DGV1.DataSource = Nothing : End If '清除T2DGV1資料

        Dim SQLQuery As String = ""
        Outtmp1 = Format(Replace(Replace("#SyncOuttmp1" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")

        SQLQuery = "  IF (OBJECT_ID('tempdb.." & Outtmp1 & "') IS NOT NULL)  DROP TABLE " & Outtmp1 & " " & vbCrLf & vbCrLf
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                '先寫暫存 Outtmp1
                同意載入()
                SQLQuery += " CREATE TABLE [" & Outtmp1 & "] (  "
                SQLQuery += " 草稿單號 VARCHAR(50) ,列號 INT,存編 VARCHAR(50),品名 VARCHAR(100),預出數量 NUMERIC(20,4),銷售 VARCHAR(6),管理 NVARCHAR(2)) " & vbCrLf
                If 同意 = "N" Or 同意 = "" Then
                    SQLQuery += " INSERT INTO [" & Outtmp1 & "] EXEC [dbo].[預_分時領料明細01v] 'ADB',N'" & 草稿T2.Text & "',N'',NULL,NULL " & vbCrLf & vbCrLf
                Else
                    SQLQuery += " INSERT INTO [" & Outtmp1 & "] EXEC [dbo].[預_分時領料明細01v] 'ADC',N'" & 草稿T2.Text & "',N'',NULL,NULL " & vbCrLf & vbCrLf
                End If
                '整合 Outtmp1 及 Outtmp4
                SQLQuery += "   SELECT CAST(S1.[草稿單號] AS VARCHAR(50)) AS '草稿單號', S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                ...
                SQLQuery += "          0 AS '單位2數量', 0 AS '單位1KG', 0 AS '小單位', 0 AS '計價單位', 0 AS '銷售金額', NULL AS '付款方法', NULL AS '員工', NULL AS '入雛日', 'N' AS '生產否', 0 AS 'BaseType', 0 AS 'BaseEntry', 0 AS 'BaseLine', 0 AS '實際金額',"
                SQLQuery += "          S1.[銷售], S1.[管理], 0.0 AS '含稅單價' "
                ...
                SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售] "
                SQLQuery += "          ) S2 ON S1.[存編] COLLATE Chinese_PRC_Ci_AS = S2.[存編] COLLATE Chinese_PRC_Ci_AS "
                SQLQuery += "              AND S1.[銷售] COLLATE Chinese_PRC_Ci_AS = S2.[銷售] COLLATE Chinese_PRC_Ci_AS "
                SQLQuery += " ORDER BY 2 "

            Case "1", "2", "5"    '1.單據領料, 2.銷售出庫, 5.寄庫出庫
                '先寫暫存 Outtmp1
                SQLQuery += "   SELECT CAST(S1.[DocEntry] AS VARCHAR(50)) AS '草稿單號', S1.[LineNum] AS '列號', S1.[ItemCode] AS '存編', S1.[Dscription] AS '品名', dbo.getRoundN(S1.[Quantity],4) AS '預出數量', "
                ...
                Select Case 作業編號 '銷售別類
                    Case "1", "3", "5"    '1.單據領料, 3.存貨領用, 5.寄庫出庫
                        SQLQuery += " '銷售' AS '銷售', "
                    Case "2"         '2.銷售出庫
                ...
                SQLQuery += "          CASE WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'Y') THEN 'B' ELSE 'X' END END END) AS '管理', "
                SQLQuery += "          S1.[PriceAfVAT] AS '含稅單價',S2.[DfltWH] AS '庫位' "
                SQLQuery += "          INTO " & Outtmp1 & " "
                ...
                Select Case 作業編號
                    Case "1"    '1.單據領料
                        SQLQuery += "   SELECT CAST(S1.[草稿單號] AS VARCHAR(50)) AS '草稿單號', S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                        SQLQuery += "          dbo.getRoundN((ISNULL((CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE ISNULL(S2.[數量],0) END),0))- S1.[預出數量],4) AS '差異數', dbo.getRoundN(ISNULL(S2.[重量],0),2) AS '出庫重量', "
                        SQLQuery += "          S1.[單位2數量], S1.[單位1KG], S1.[小單位], S1.[計價單位], S1.[銷售金額], S1.[付款方法], S1.[員工], S1.[入雛日], S1.[生產否], S1.[BaseType], S1.[BaseEntry], S1.[BaseLine], 0 AS '實際金額', S1.[銷售], S1.[管理], 0.0 AS '含稅單價', S1.[庫位] "
                        SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN ("
                        SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售] "
                        SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售] ) S2 ON S1.[存編] = S2.[存編] COLLATE Chinese_PRC_Ci_AS AND S1.[銷售] = S2.[銷售] COLLATE Chinese_PRC_Ci_AS ORDER BY 2 "

                    Case "2", "5"    '2.銷售出庫, 5.寄庫出庫
                        SQLQuery += "   SELECT CAST(S1.[草稿單號] AS VARCHAR(50)) AS '草稿單號', S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                        SQLQuery += "          dbo.getRoundN((ISNULL((CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE ISNULL(S2.[數量],0) END),0))- S1.[預出數量],4) AS '差異數', dbo.getRoundN(ISNULL(S2.[重量],0),2) AS '出庫重量', "
                        SQLQuery += "          S1.[單位2數量], S1.[單位1KG], S1.[小單位], S1.[計價單位], CAST(S1.[銷售金額] AS NUMERIC(20,4)) AS '銷售金額', S1.[付款方法], S1.[員工], S1.[入雛日], S1.[生產否], S1.[BaseType], S1.[BaseEntry], S1.[BaseLine], "
                        Select Case 作業編號
                            Case "2"
                                SQLQuery += "    CAST(CASE S1.[計價單位] WHEN '0' THEN CAST(S1.[銷售金額] AS NUMERIC(20,4)) "
                                SQLQuery += "                            WHEN '1' THEN NULLIF((CAST(S1.[銷售金額] AS NUMERIC(20,4)) * CAST(S2.[重量]      AS NUMERIC(20,4)) / CAST(S2.[數量] AS NUMERIC(20,4))),0) "
                                SQLQuery += "                            WHEN '2' THEN         CAST(S1.[銷售金額] AS NUMERIC(20,4)) * CAST(S1.[單位2數量] AS NUMERIC(20,4)) / NULLIF(CASE WHEN S1.[管理] = 'F' THEN CAST(S1.[預出數量] AS NUMERIC(20,4)) ELSE CAST(ISNULL(S2.[數量],0) AS NUMERIC(20,4)) END,0) "
                                SQLQuery += "         ELSE 0 END AS NUMERIC(20,4)) AS '實際金額', "
                            Case "5"
                        ...
                        End Select
                        SQLQuery += "          S1.[銷售], S1.[管理], CAST(S1.[含稅單價] AS NUMERIC(20,4)) AS '含稅單價', S1.[庫位] "
                        SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN ("
                        SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售] "
            ...
            Case "3"    '3.存貨領用
                '先寫暫存 Outtmp1
                If 存領庫位 = "N" Then
                    SQLQuery += "       SELECT CAST(S1.[ID] AS VARCHAR(50)) AS '草稿單號', (ROW_NUMBER() OVER (PARTITION BY S1.[ID] ORDER BY S1.[ID] ASC, S1.[ProCode] ASC) - 1) AS '列號', "
                    SQLQuery += "              S1.[ProCode] AS '存編', S1.[ProName] AS '品名',dbo.getRoundN(S1.[Num],4) AS '預出數量', '銷售' AS '銷售', "
                    SQLQuery += "             (CASE WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'N') THEN 'F' "
                    SQLQuery += "                   WHEN (S2.[ManSerNum] = 'Y' AND S2.[ManBtchNum] = 'N') THEN 'S' "
                    SQLQuery += "                   WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'Y') THEN 'B' ELSE 'X' END ) AS '管理', "
                    SQLQuery += "              S2.[DfltWH] AS '庫位' INTO " & Outtmp1 & " "
                    SQLQuery += "         FROM [KS_Z_StockApply_Detail] S1 LEFT JOIN [OITM] S2 ON S1.[ProCode] = S2.[ItemCode] "
                    SQLQuery += "        WHERE S1.[ID] = '" & 草稿T2.Text & "' "
                Else
                    '20180105增加調整依實際庫位整合比對
                    SQLQuery += "       SELECT CAST(S1.[ID] AS VARCHAR(50)) AS '草稿單號', (ROW_NUMBER() OVER (PARTITION BY S1.[ID] ORDER BY S1.[ID] ASC, S1.[ProCode] ASC) - 1) AS '列號', "
                    'SQLQuery += "              S1.[ProCode] AS '存編', S1.[ProName] AS '品名',dbo.getRoundN(S1.[Num],4) AS '預出數量', '銷售' AS '銷售', "
                    SQLQuery += "              S1.[ProCode] AS '存編', S1.[ProName] AS '品名',S3.[數量] AS '預出數量', '銷售' AS '銷售', "
                    SQLQuery += "             (CASE WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'N') THEN 'F' "
                    SQLQuery += "                   WHEN (S2.[ManSerNum] = 'Y' AND S2.[ManBtchNum] = 'N') THEN 'S' "
                    SQLQuery += "                   WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'Y') THEN 'B' ELSE 'X' END ) AS '管理', "
                    SQLQuery += "              S3.[庫位] AS '庫位' INTO " & Outtmp1 & " "
                    SQLQuery += "         FROM [KS_Z_StockApply_Detail] S1 LEFT  JOIN [OITM] S2 ON S1.[ProCode] = S2.[ItemCode] "
                    SQLQuery += "                                          LEFT  JOIN "
                    SQLQuery += "          ("
                    SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售], T1.[庫位] "
                    SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售], T1.[庫位] "
                    SQLQuery += "          ) S3 "
                    SQLQuery += "               ON S1.[ProCode] COLLATE Chinese_Taiwan_Stroke_CI_AS = S3.[存編] COLLATE Chinese_Taiwan_Stroke_CI_AS "
                    SQLQuery += "        WHERE S1.[ID] = '" & 草稿T2.Text & "' "
                End If
                '整合 Outtmp1 及 Outtmp4
                SQLQuery += "   SELECT CAST(S1.[草稿單號] AS VARCHAR(50))  AS '草稿單號', S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                SQLQuery += "          dbo.getRoundN((ISNULL((CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE ISNULL(S2.[數量],0) END),0))- S1.[預出數量],4) AS '差異數', dbo.getRoundN(ISNULL(S2.[重量],0),2) AS '出庫重量', "
                SQLQuery += "          0 AS '單位2數量', 0 AS '單位1KG', 0 AS '小單位', 0 AS '計價單位', 0 AS '銷售金額', NULL AS '付款方法', NULL AS '員工', NULL AS '入雛日', 'N' AS '生產否', 0 AS 'BaseType', 0 AS 'BaseEntry', 0 AS 'BaseLine', 0 AS '實際金額', "
                SQLQuery += "          S1.[銷售], S1.[管理], 0.0 AS '含稅單價', S1.[庫位] "
                SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN "
                SQLQuery += "          ("
                SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售], T1.[庫位] "
                SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售], T1.[庫位] "
                SQLQuery += "          ) S2 ON S1.[存編] COLLATE Chinese_Taiwan_Stroke_CI_AS = S2.[存編] COLLATE Chinese_Taiwan_Stroke_CI_AS "
                SQLQuery += "              AND S1.[銷售] COLLATE Chinese_Taiwan_Stroke_CI_AS = S2.[銷售] COLLATE Chinese_Taiwan_Stroke_CI_AS "
                SQLQuery += "              AND S1.[庫位] COLLATE Chinese_Taiwan_Stroke_CI_AS = S2.[庫位] COLLATE Chinese_Taiwan_Stroke_CI_AS "
                SQLQuery += " ORDER BY 2 "
    ...
    Private Sub 同意載入()
        If 同意 <> "" Then : ks1DataSetDGV.Tables("同意").Clear() : End If

        Dim SQLADD As String = ""
        SQLADD = "    SELECT TOP 1 ISNULL([項目同意],'N') AS '同意', CASE [項目同意] WHEN 'Y' THEN '同意' WHEN 'N' THEN '未決' WHEN 'D' THEN '不同意' ELSE 'XX' END AS '同意否' "
        SQLADD += "     FROM [KaiShingPlug].[dbo].[倉_分時項目確認] "
        SQLADD += "    WHERE [分時領料單] = '" & 草稿T2.Text & "' AND [項目同意] IN ('Y','N') "
        SQLADD += " GROUP BY [項目同意] "
        ...
        Try
            ...
            DataAdapterDGV = New SqlDataAdapter(SQLCmd.CommandText, DBConn)
            DataAdapterDGV.Fill(ks1DataSetDGV, "同意")

            If ks1DataSetDGV.Tables("同意").Rows.Count > 0 Then
                同意 = ks1DataSetDGV.Tables("同意").Rows(0).Item("同意")
                La2申請.Text = ks1DataSetDGV.Tables("同意").Rows(0).Item("同意否")
            Else
                同意 = ""
                La2申請.Text = "."
            End If
            ...
            Select Case 作業編號
                ...
                Case "0", "1", "3"    '1.單據領料, 3.存貨領用
                    Select Case column.Name
                        ...
                        Case "庫位" : column.HeaderText = "庫位" : column.DisplayIndex = 6
                        Case "列號" : column.HeaderText = "列號" : column.DisplayIndex = 7
                        Case Else
                            column.Visible = False
                    End Select
                Case "2", "5"    '2.銷售出庫, 5.寄庫出庫
    ...
    Private Sub T2查詢單據非單據品項()
        If T2DGV3.RowCount > 0 Then : ks1DataSetDGV.Tables("T2DGV3").Reset() : T2DGV3.DataSource = Nothing : End If '清除T2DGV3資料
        Dim SQLQuery As String = "" ': Dim Outtmp5 As String = ""
        'SQLQuery = " SELECT T0.[存編], T0.[品名] FROM " & Outtmp4 & " T0 EXCEPT SELECT T0.[存編], T0.[品名] FROM " & Outtmp1 & " T0 "  原始

        Select Case 作業編號
            Case "0"  '0.排程領料
                SQLQuery = "  SELECT A.[存編], A.[品名], B.[數量]"
                SQLQuery += " FROM( SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS AS '存編', T0.[品名] COLLATE Chinese_PRC_Ci_AS AS '品名' FROM " & Outtmp4 & " T0 EXCEPT SELECT T0.[存編], T0.[品名] FROM " & Outtmp1 & " T0) A INNER JOIN "
                SQLQuery += "     ( SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS AS '存編', T0.[品名] COLLATE Chinese_PRC_Ci_AS AS '品名', SUM(T0.[數量]) AS '數量' "
                SQLQuery += "         FROM " & Outtmp4 & " T0 GROUP BY T0.[存編] COLLATE Chinese_PRC_Ci_AS, T0.[品名] COLLATE Chinese_PRC_Ci_AS   ) B ON A.[存編] = B.[存編] "

            Case "1", "2", "5"    '1.單據領料, '2.銷售出庫, 3.存貨領用, 5.寄庫出庫
                '--20160906雅淳修改，增加數量統計
                SQLQuery = "  SELECT A.[存編], A.[品名], B.[數量]"
                SQLQuery += " FROM( SELECT T0.[存編], T0.[品名] FROM " & Outtmp4 & " T0 EXCEPT SELECT T0.[存編], T0.[品名] FROM " & Outtmp1 & " T0) A INNER JOIN "
                SQLQuery += "     ( SELECT T0.[存編], T0.[品名], SUM(T0.[數量]) AS '數量' FROM " & Outtmp4 & " T0 GROUP BY T0.[存編], T0.[品名]   ) B ON A.[存編] COLLATE Chinese_PRC_Ci_AS = B.[存編] COLLATE Chinese_PRC_Ci_AS "
                '--
            Case "3"     '3.存貨領用
                SQLQuery = "  SELECT A.[存編], A.[品名], B.[庫位], B.[數量] "
                SQLQuery += " FROM( SELECT T0.[存編], T0.[品名], T0.[庫位] FROM " & Outtmp4 & " T0 EXCEPT SELECT T0.[存編], T0.[品名], T0.[庫位] FROM " & Outtmp1 & " T0) A INNER JOIN "
                SQLQuery += "     ( SELECT T0.[存編], T0.[品名], T0.[庫位], SUM(T0.[數量]) AS '數量' FROM " & Outtmp4 & " T0 GROUP BY T0.[存編], T0.[品名], T0.[庫位] ) B ON A.[存編] COLLATE Chinese_PRC_Ci_AS = B.[存編] COLLATE Chinese_PRC_Ci_AS "
    ...
    Private Sub T1DGV2_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles T1DGV2.CellClick
        ...
        Select Case 移除作業T1.Text
            Case "24.分時電宰" : 查詢別號 = " =   '24'" : 查詢編號 = "0"       '領料  24,25
    ...
    Private Sub T2條碼出庫更新作業()
        ...
        Try
            ...
            SQLCmd.CommandText += "  FROM [KaiShingPlug].[dbo].[PDA_OutData_SAP] S0 INNER JOIN " & Outtmp2 & " S1 ON S0.[OUT02] = S1.[DistNumber] AND [OUT06] = '3' AND [OUT04] = '" & 草稿T2.Text & "' " & vbCrLf

            Select Case 作業編號
                Case "0"    '
                    SQLCmd.CommandText += " UPDATE [KaiShingPlug].[dbo].[倉_分時領料] SET [啟用否] = 'S',[發貨單號] = '" & 回傳單號 & "',[發貨時間] = GETDATE() "
                    SQLCmd.CommandText += "   FROM [KaiShingPlug].[dbo].[倉_分時領料] "
                    SQLCmd.CommandText += "  WHERE [分時領料單] = '" & 草稿T2.Text & "' AND [啟用否] = 'Y'" & vbCrLf
                Case "3"    '3.存貨領用 ID = 2 已出庫
                    SQLCmd.CommandText += " UPDATE [KS_Z_StockApply] SET [Examine] = '2',[DocNum] = '" & 回傳單號 & "',[SendOutDate] = GETDATE() "
                    SQLCmd.CommandText += "   FROM [KS_Z_StockApply] WHERE [ID] = '" & 草稿T2.Text & "' " & vbCrLf
            End Select
        ...
        Select Case 異常作業
            ...
            Case "非單品項"
                Select Case 作業編號
                    ...
                    Case "0", "1", "2", "5"    '1.單據領料, 2.銷售出庫, 4.寄庫出庫
            ...
            Select Case 異常作業
                Case "列出全部", "列出重覆", "列出已出", "變更銷售"
                    Select Case column.Name
                        ...
                        Case "保存日期" : column.HeaderText = "保存日期" : column.DisplayIndex = 7
                        Case "製造單號" : column.HeaderText = "製造單號" : column.DisplayIndex = 8
                        Case "出庫" : column.HeaderText = "出庫" : column.DisplayIndex = 9
                        Case "狀態" : column.HeaderText = "狀態" : column.DisplayIndex = 10
                        Case "銷售" : column.HeaderText = "銷售" : column.DisplayIndex = 11
                        Case Else
                            column.Visible = False
                    End Select
                Case "非單品項"
    ...
    Private Sub SentToPrinter(ByVal olabel As LabeData)
        ...
        Dim SN1 As Int16 = 0 : Dim SN2 As Int16 = 0
        ...
        Else
            ...
            For Each Chr As Char In olabel.Itemname
                ...
                Select Case 作業編號     '0.排程領料
                    Case "0"
                        SN1 = 20 : SN2 = 46
                    Case Else
                        SN1 = 14 : SN2 = 53
                End Select
                If Counter <= SN1 Then   '超過14個字元分二列
            ...
            Call windowsfont(40, 35, SN2, 0, 2, 0, "新細明體", Itemname1) '列印文字  x,y,高,旋轉角度,(粗、斜), 底線,字型,內容
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                Call barcode(T1.Text, T2.Text, "128", T3.Text, "1", "0", "2", "2", olabel.Barcode) '列印條碼 x,y,條碼類型,高度,是否列印條碼碼文,條碼旋轉角度,條碼窄,條碼寬, 容 以點(point)表示
            Case Else
