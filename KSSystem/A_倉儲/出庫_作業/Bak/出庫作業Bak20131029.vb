...
Public Class 出庫作業Bak20131029
    ...
    Private Sub 出庫作業Bak20131029_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        區別.Text = ""               '目前區別
        草稿.Text = ""               '目前草稿
        ComboBox1.SelectedIndex = -1    '作業選項
        ...
        For i As Integer = 0 To ExcelDGV.RowCount - 1
            If ExcelDGV.Rows(i).Cells("F1").FormattedValue <> "" Then
                ...
                If Format(Replace(Mid(ExcelDGV.Rows(i).Cells("F1").FormattedValue, 19, 5 + 0), " ", ""), "") = vbNullString Then
                    F4 = 0
                Else
                    F4 = Format(Replace(Mid(ExcelDGV.Rows(i).Cells("F1").FormattedValue, 19, 5 + 0), " ", ""), "")
                End If
    ...
    Private Sub 查詢條碼區別()
        ...
        Try
            ...
            If 區別F3 = "" Then
                sWhere_1 = "    WHERE [F3] IN ('A','I')      AND [Del] = 'N'"
            Else
    ...
    Private Sub 查詢條碼區別明細()
        ...
        Try
            ...
            If 電宰Rb.Checked = True Then
                ...
                SQLCmd.CommandText += "            WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'A' AND TX.[F1] = @F1 AND TX.[F2] = T1.[DistNumber] AND TX.[Del] = 'N' ) "
                ...
                SQLCmd.CommandText += "    WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE T0.[F3] = 'A' AND TX.[F1] = @F1 AND TX.[F2] = T0.[F2] AND T0.[Del] = 'N' ) "
            ...
            Else
                ...
                SQLCmd.CommandText += "   SELECT T1.[ItemCode] AS '存編', T1.[ItemName] AS '品名', T0.[F2] AS '條碼', CAST(ISNULL(T1.[Quantity],9999) AS NUMERIC(5,0)) AS '數量', T1.[U_M1] AS '重量', T1.[U_M2] AS '儲位', T1.[MnfDate] AS '製造日期', T1.[U_M8] AS '製造單號', (CASE WHEN COUNT(ISNULL(T0.[F2],0)) > 1 THEN '重覆' ELSE CASE WHEN ISNULL(T1.[Quantity],'9999') = '9999' THEN '未入庫' ELSE CASE WHEN T1.[Quantity] = 0 THEN '出庫' ELSE CASE  WHEN T1.[Quantity] >= 1 THEN '庫存' ELSE '不知' END END END END) AS '狀態', ''                    AS 'G1' "
                SQLCmd.CommandText += "     FROM [Z_KS_Barcode] T0 LEFT  JOIN "
                SQLCmd.CommandText += "         ( SELECT T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], SUM(ISNULL(T2.[Quantity],9999)) AS 'Quantity', T1.[U_M1], T1.[U_M8], T1.[MnfDate], T3.[SalPackUn] "
        ...
        Try
            ...
            Select Case ComboBox2.SelectedIndex
                Case "0"    '重覆
                    ...
                    SQLCmd.CommandText += "    WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE T0.[F3] = 'A' AND TX.[F2] = T0.[F2] AND T0.[Del] = 'N' GROUP BY TX.[F2] HAVING COUNT(ISNULL(TX.[F2],0)) > 1 ) AND T0.[F3] = 'A' AND T0.[Del] = 'N' "
                ...
                Case "1"    '出庫
                    ...
                    SQLCmd.CommandText += "             FROM [OSRN] T1 INNER JOIN [OSRQ] T2 ON T1.[ItemCode] = T2.[ItemCode] AND T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry] "
                    SQLCmd.CommandText += "                            INNER JOIN [OITM] T3 ON T1.[ItemCode] = T3.[ItemCode] "
                    SQLCmd.CommandText += "            WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'A' AND TX.[F2] = T1.[DistNumber] AND TX.[Del] = 'N' ) "
                    SQLCmd.CommandText += "         GROUP BY T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], T1.[U_M1], T1.[U_M8], T1.[MnfDate] "
                    SQLCmd.CommandText += "         ) T1 ON T0.[F2] = T1.[DistNumber] "
                    SQLCmd.CommandText += "     WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE T0.[F2] = TX.[F2] AND T0.[F3] = 'A' AND T0.[Del] = 'N' ) AND T1.[Quantity] = 0 "
                    SQLCmd.CommandText += "  ORDER BY T0.[F2] , T0.[Sid] "
                Case "2"    '全部
                    If 區別.Text = "" Then
                        MsgBox("未選取查詢區別", 48, "警告")
                    ...
                    SQLCmd.CommandText += "             FROM [OSRN] T1 INNER JOIN [OSRQ] T2 ON T1.[ItemCode] = T2.[ItemCode] AND T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry] "
                    SQLCmd.CommandText += "                            INNER JOIN [OITM] T3 ON T1.[ItemCode] = T3.[ItemCode] "
                    SQLCmd.CommandText += "            WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE TX.[F1] = @F1 AND TX.[F3] = 'A' AND TX.[F2] = T1.[DistNumber] AND TX.[Del] = 'N' ) "
                    SQLCmd.CommandText += "         GROUP BY T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], T1.[U_M1], T1.[U_M8], T1.[MnfDate] "
                    SQLCmd.CommandText += "         ) T1 ON T0.[F2] = T1.[DistNumber] "
                    SQLCmd.CommandText += "     WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE TX.[F1] = @F1 AND TX.[F2] = T0.[F2] AND T0.[F3] = 'A' AND T0.[Del] = 'N') "
                    SQLCmd.CommandText += " ORDER BY T0.[F2] , T0.[Sid] "
    ...
    Private Sub 查詢比對非草稿上之品項()
        ...
        Try
            ...
            If ComboBox1.SelectedIndex = "0" Or ComboBox1.SelectedIndex = "1" Then
                ...
                SQLCmd.CommandText += "                            LEFT JOIN [OITM] T3 ON T1.[ItemCode] = T3.[ItemCode]     "
                SQLCmd.CommandText += "     WHERE EXISTS( SELECT * FROM [Z_KS_Barcode] TX WHERE TX.[F4] = @DocEntry AND T0.[F3] = 'A' AND T1.[DistNumber] = TX.[F2] AND TX.[Del] = 'N' ) AND T0.[F4] = @DocEntry AND T0.[Del] = 'N' AND ISNULL(S1.[ItemCode],'') = '' "
            ...
            ElseIf ComboBox1.SelectedIndex = "2" Then
                ...
                SQLCmd.CommandText += "                            LEFT JOIN [OITM] T3 ON T1.[ItemCode] = T3.[ItemCode]     "
                SQLCmd.CommandText += "     WHERE EXISTS( SELECT * FROM [Z_KS_Barcode] TX WHERE TX.[F4] = @DocEntry AND T0.[F3] = 'A' AND T1.[DistNumber] = TX.[F2] AND TX.[Del] = 'N' ) AND T0.[F4] = @DocEntry AND T0.[Del] = 'N' AND ISNULL(S1.[ProCode],'') = '' "
                SQLCmd.CommandText += "  GROUP BY T0.[F1], T1.[ItemCode], T3.[ItemName] "
            End If
    ...
    Private Sub 查詢條碼區別明細_草稿()
        ...
        Try
            ...
            'SQLCmd.CommandText = "    SELECT [F2] AS '條碼', COUNT([F2]) AS '數量'"
            'SQLCmd.CommandText += "     FROM [Z_KS_Barcode] "
            'SQLCmd.CommandText += "    WHERE [F1] = '" & DGV2.CurrentRow.Cells("區別").Value & "' AND [Del] = 'N' "
            'SQLCmd.CommandText += " GROUP BY [F2] "
            'SQLCmd.CommandText += " ORDER BY COUNT([F2]) DESC ,[F2] "

            ''SQLCmd.CommandText = "    SELECT T1.[ItemCode] AS '存編', T3.[ItemName] AS '品名', T0.[F2] AS '條碼', '' AS '重量', '' AS '儲位', T1.[MnfDate] AS '製造日期', '' AS '製造單號', (CASE WHEN COUNT(ISNULL(T0.[F2],0)) > 1 THEN '重覆' ELSE CASE WHEN ISNULL(T2.[Quantity],'9999') = '9999' THEN '未入庫' ELSE CASE WHEN T2.[Quantity] = 0 THEN '出庫' ELSE CASE  WHEN T2.[Quantity] >= 1 THEN '庫存' ELSE '不知' END END END END) AS '狀態' "
            ''SQLCmd.CommandText += "     FROM [Z_KS_Barcode] T0 LEFT JOIN [OSRN] T1 ON T0.[F2]        = T1.[DistNumber] "
            ''SQLCmd.CommandText += "                            LEFT JOIN [OSRQ] T2 ON T1.[SysNumber] = T2.[SysNumber] AND T1.[ItemCode] = T2.[ItemCode] "
            ''SQLCmd.CommandText += "                            LEFT JOIN [OITM] T3 ON T1.[ItemCode]  = T3.[ItemCode] "
            ''SQLCmd.CommandText += "    WHERE EXISTS( SELECT * FROM [Z_KS_Barcode] TX WHERE TX.[F4] = '" & Label12.Text & "' AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' ) AND T0.[F4] = '" & Label12.Text & "' AND T0.[Del] = 'N' "
            ''SQLCmd.CommandText += " GROUP BY T1.[ItemCode], T3.[ItemName], T2.[Quantity], T1.[MnfDate], T0.[F2] "
            ''SQLCmd.CommandText += " ORDER BY COUNT(T0.[F2]) DESC ,T0.[F2] "
            ...
            If ComboBox1.SelectedIndex = "0" Or ComboBox1.SelectedIndex = "1" Or ComboBox1.SelectedIndex = "2" Then
                ...
                'Select Case ComboBox1.SelectedIndex
                '    Case "0"    '銷售出庫
                ''SQLCmd.CommandText += "   SELECT T1.[ItemCode] AS '存編', T3.[ItemName] AS '品名', T0.[F2] AS '條碼', T1.[U_M1] AS '重量', T1.[U_M2] AS '儲位', T1.[MnfDate] AS '製造日期', T1.[U_M8] AS '製造單號', (CASE WHEN COUNT(ISNULL(T0.[F2],0)) > 1 THEN '重覆' ELSE CASE WHEN ISNULL(T2.[Quantity],'9999') = '9999' THEN '未入庫' ELSE CASE WHEN T2.[Quantity] = 0 THEN '出庫' ELSE CASE  WHEN T2.[Quantity] >= 1 THEN '庫存' ELSE '不知' END END END END) AS '狀態', ISNULL(T4.[M_NO],'') AS 'G1' "
                ''SQLCmd.CommandText += "     FROM [Z_KS_Barcode] T0 LEFT JOIN [OSRN]       T1 ON T0.[F2]        = T1.[DistNumber] "
                ''SQLCmd.CommandText += "                            LEFT JOIN [OSRQ]       T2 ON T1.[SysNumber] = T2.[SysNumber] AND T1.[ItemCode] = T2.[ItemCode] "
                ''SQLCmd.CommandText += "                            LEFT JOIN [OITM]       T3 ON T1.[ItemCode]  = T3.[ItemCode] "
                '' ''SQLCmd.CommandText += "     FROM [Z_KS_Barcode] T0 LEFT  JOIN [OSRN]      T1 ON T0.[F2]       = T1.[DistNumber] "
                '' ''SQLCmd.CommandText += "                            INNER JOIN [OSRQ]      T2 ON T1.[ItemCode] = T2.[ItemCode] AND T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry]  "
                '' ''SQLCmd.CommandText += "                            INNER JOIN [OITM]      T3 ON T1.[ItemCode] = T3.[ItemCode] "
                ''SQLCmd.CommandText += "                            LEFT JOIN [z_format_G] T4 ON T1.[U_M8]     = T4.[M_NO] "
                ''SQLCmd.CommandText += "    WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE EXISTS( SELECT TX.[F1] FROM [Z_KS_Barcode] TX WHERE TX.[F4] = @DocEntry AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' GROUP BY TX.[F1]) AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' ) AND "
                ''SQLCmd.CommandText += "          EXISTS( SELECT TX.[F1] FROM [Z_KS_Barcode] TX WHERE TX.[F4] = @DocEntry AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' GROUP BY TX.[F1] ) AND T0.[Del] = 'N' "
                ''SQLCmd.CommandText += " GROUP BY T1.[ItemCode], T3.[ItemName], T2.[Quantity], T1.[MnfDate], T0.[F2], T1.[U_M1], T1.[U_M2], T1.[U_M8], T4.[M_NO] "
                ''SQLCmd.CommandText += " ORDER BY COUNT(T0.[F2]) DESC ,T0.[F2] "
                'End Select

                'SQLCmd.CommandText += "    WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE EXISTS( SELECT TX.[F1] FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'A' AND TX.[F4] = @DocEntry AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' GROUP BY TX.[F1]) AND T0.[F3] = 'A' AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' ) AND "
                'SQLCmd.CommandText += "          EXISTS( SELECT TX.[F1] FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'A' AND TX.[F4] = @DocEntry AND T0.[F2] = TX.[F2] AND TX.[Del] = 'N' GROUP BY TX.[F1] ) AND T0.[F3] = 'A' AND T0.[Del] = 'N' "
                ...
                SQLCmd.CommandText += "             FROM [OSRN] T1 INNER JOIN [OSRQ] T2 ON T1.[ItemCode] = T2.[ItemCode] AND T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry] "
                SQLCmd.CommandText += "                            INNER JOIN [OITM] T3 ON T1.[ItemCode] = T3.[ItemCode] "
                SQLCmd.CommandText += "            WHERE EXISTS( SELECT TX.[F2] FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'A' AND TX.[F4] = @DocEntry AND TX.[F2] = T1.[DistNumber] AND TX.[Del] = 'N' ) "
                SQLCmd.CommandText += "         GROUP BY T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], T1.[U_M1], T1.[U_M8], T1.[MnfDate] "
                SQLCmd.CommandText += "         )               T1 ON T0.[F2] = T1.[DistNumber] "
                SQLCmd.CommandText += "                            LEFT  JOIN [z_format_G] T4 ON T1.[U_M8] = T4.[M_NO] "
                SQLCmd.CommandText += "    WHERE EXISTS( SELECT TX.[F1] FROM [Z_KS_Barcode] TX WHERE T0.[F3] = 'A' AND TX.[F4] = @DocEntry AND TX.[F2] = T0.[F2] AND T0.[Del] = 'N' GROUP BY TX.[F1] ) "
                SQLCmd.CommandText += " GROUP BY T1.[ItemCode], T1.[ItemName], T1.[Quantity], T1.[MnfDate], T0.[F2], T1.[U_M1], T1.[U_M2], T1.[U_M8], T4.[M_NO] "
                SQLCmd.CommandText += " ORDER BY COUNT(T0.[F2]) DESC ,T0.[F2] "
            End If
            SQLCmd.Parameters.Clear()
    ...
    Private Sub 發貨作業_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 發貨作業.Click
        ...
        ElseIf ComboBox1.SelectedIndex = "2" Then '存領出庫
            存條碼至待發貨區_存領()
            SyncToSAP2()
        End If
    ...
    Private Sub 存條碼至待發貨區()
        ...
        If ComboBox1.SelectedIndex = "0" Or ComboBox1.SelectedIndex = "1" Then      'Case "0"    銷售出庫  Case "1"    寄庫品出庫
    ...
    Private Sub 存條碼至待發貨區_存領()
        ...
        If ComboBox1.SelectedIndex = "2" Then  '存領出庫
            SQLCmd.CommandText += " INSERT INTO [@ksOBList] ([OB01],[OB02],[OB03],[OB04],[OB05],[OB07],[OB08],[OB09],[OB10],[OB12]) "
            SQLCmd.CommandText += " SELECT S1.[OB01], S1.[OB02], S1.[OB03], S1.[OB04], '1' AS 'OB05', @TypeSelect AS 'OB07', @DocEntry AS 'OB08', S0.[LineNum]  AS 'OB09', '3' AS 'OB10', '" & OB12Date & "' AS 'OB12' "
