...
Public Class A_文件查庫存_備留_20130806   '營業發貨及生產領料
    ...
    Private Sub A_文件查庫存_備留_20130806_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub 存入_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 存入.Click
        ...
        For Each oRow As DataGridViewRow In DGV2.SelectedRows
            ...
            ''TX.[存編], TX.[品名], TX.[倉庫], ISNULL(TX.[儲位],'') AS '儲位', SUM(ISNULL(TX.[數量],0)) AS '數量', TX.[單位], SUM(ISNULL(TX.[重量],0)) AS '重量', ISNULL(TX.[製造日期],'') AS '製造日期', ISNULL(TX.[天數],'') AS '天數', TX.[製造單號], TX.[G1], TX.[G2], TX.[G_OK], SUM(ISNULL(TX.[數量2],0)) AS '2號庫數量', SUM(ISNULL(TX.[重量2],0)) AS '2號庫重量' "

            If TextBox1.Text = "" Then
                'If Format(oRow.Cells("數量").Value, "") <> 0 Then
                TextBox1.Text = TextBox1.Text + "儲 " + Format(oRow.Cells("儲位").Value, "") + " 數 " + Format(oRow.Cells("數量").Value, "##0") + " 日 " + Format(oRow.Cells("製造日期").Value, "")
            Else
                TextBox1.Text = TextBox1.Text + "," + vbCrLf + "儲 " + Format(oRow.Cells("儲位").Value, "") + " 數 " + Format(oRow.Cells("數量").Value, "##0") + " 日 " + Format(oRow.Cells("製造日期").Value, "") '+ vbCrLf
                'End If
            End If
