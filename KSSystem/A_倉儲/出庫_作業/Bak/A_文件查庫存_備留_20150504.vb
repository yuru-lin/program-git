...
Public Class A_文件查庫存_Bak_20150504   '營業發貨及生產領料
        ...
        Try
            ...
            Select Case DGV1.CurrentRow.Cells("別類").Value
                Case "序號"
                    ...
                    SQLCmd.CommandText += "          ISNULL(TX.[磅入日期],'') AS '磅入日期', ISNULL(TX.[有效日期],'') AS '有效日期', ISNULL(TX.[天數],'') AS '天數', ISNULL(CAST(CASE WHEN DATEDIFF(DAY,GETDATE(),TX.[有效日期])+1 BETWEEN '0' AND '90' THEN DATEDIFF(DAY,GETDATE(),TX.[有效日期])+1 END AS NVARCHAR(5)),'') AS '剩餘天數', "
                    SQLCmd.CommandText += "          TX.[製造單號], ISNULL(TX.[生產日期],'') AS '生產日期', TX.[G1], TX.[G2], TX.[G_OK], SUM(ISNULL(TX.[數量2],0)) AS '2號庫數量', SUM(ISNULL(TX.[重量2],0)) AS '2號庫重量' "
                    SQLCmd.CommandText += "     FROM ( "
                    SQLCmd.CommandText += "           SELECT T0.[ItemCode] AS '存編', T0.[ItemName] AS '品名', T0.[WhsCode] AS '倉庫', T0.[U_M2] AS '儲位', "
                    SQLCmd.CommandText += "                  T0.[Quantity] AS '數量', T0.[InvntryUom] as '單位', T0.[U_M1] AS '重量', T0.[InDate] AS '磅入日期', "
                    ...
                    SQLCmd.CommandText += "                          SUM(CAST(ISNULL(T0.[U_M1],0)     AS NUMERIC(19,2))) AS 'U_M1',CONVERT(CHAR(10),T0.[InDate], 120) AS 'InDate', CONVERT(CHAR(10),T0.[MnfDate], 120) AS 'MnfDate',  "
                    ...
                    SQLCmd.CommandText += "                 GROUP BY T0.[ItemCode], T2.[ItemName], T1.[WhsCode], T0.[U_M2], T2.[InvntryUom], T0.[InDate], T0.[MnfDate], T0.[ExpDate], T0.[DistNumber], T0.[U_M8] "
                    ...
                    SQLCmd.CommandText += "        GROUP BY T0.[ItemCode], T0.[ItemName], T0.[WhsCode], T0.[U_M2], T0.[InvntryUom], T0.[InDate], T0.[MnfDate], T0.[ExpDate], T0.[DistNumber], T0.[U_M8], T0.[Quantity], T0.[U_M1], T4.[M_NO], T6.[Kind], T5.[M_NO], T7.[InvSto] "
                    SQLCmd.CommandText += "          ) TX "
                    SQLCmd.CommandText += "GROUP BY TX.[存編], TX.[品名], TX.[倉庫], TX.[儲位], TX.[單位], TX.[磅入日期], TX.[有效日期], TX.[天數], TX.[製造單號], TX.[生產日期], TX.[G1], TX.[G2], TX.[G_OK] "
                ...
                Case "批次"
                    ...
                    SQLCmd.CommandText += "        GROUP BY T0.[ItemCode], T0.[ItemName], T0.[WhsCode], T0.[U_M2], T0.[InvntryUom], T0.[InDate], T0.[MnfDate], T0.[ExpDate], T0.[DistNumber], T0.[U_M8], T0.[Quantity], T0.[U_M1], T7.[InvSto] "
                    SQLCmd.CommandText += "          ) TX "
                    SQLCmd.CommandText += "GROUP BY TX.[存編], TX.[品名], TX.[倉庫], TX.[儲位], TX.[單位], TX.[磅入日期], TX.[有效日期], TX.[天數], TX.[製造單號], TX.[生產日期], TX.[G1], TX.[G2], TX.[G_OK] "
                    SQLCmd.CommandText += "ORDER BY 5, 2"
            End Select
    ...
    Private Sub 存入_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 存入.Click
        ...
        For Each oRow As DataGridViewRow In DGV2.SelectedRows
            If TextBox1.Text = "" Then
                ...
                'TextBox1.Text = TextBox1.Text + "儲 " + Format(oRow.Cells("儲位").Value, "") + " 數 " + Format(oRow.Cells("數量").Value, "##0") + " 日 " + Format(oRow.Cells("磅入日期").Value, "")
            Else
                ...
                'TextBox1.Text = TextBox1.Text + "," + vbCrLf + "儲 " + Format(oRow.Cells("儲位").Value, "") + " 數 " + Format(oRow.Cells("數量").Value, "##0") + " 日 " + Format(oRow.Cells("磅入日期").Value, "") '+ vbCrLf
            End If
        Next
