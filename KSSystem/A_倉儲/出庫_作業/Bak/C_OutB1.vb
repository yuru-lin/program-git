...
Public Class C_OutB1
    ...
    Private Sub C_OutB1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub dgvSourceMain_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvSourceMain.CellClick
        ...
        Label12.Text = dgvSourceMain.CurrentRow.Cells("DocEntry").Value
    ...
    Private Sub LoadBarCode()
        Try
            ...
            sql = " DECLARE @DocEntry   INT  "
            sql += "DECLARE @LineNum    numeric(3,1) "
            sql += "DECLARE @TypeSelect varchar(5)   "
            sql += "SET NOCOUNT ON "
            sql += "SET @DocEntry   = '" & dgvSourceList.CurrentRow.Cells("DocEntry").Value & "' "
            sql += "SET @LineNum    = '" & dgvSourceList.CurrentRow.Cells("LineNum").Value & "'  "
            sql += "SET @TypeSelect = '" & TypeSelect & "'  "
            ...
            If oBorS = "B" Then
                ...
                sql += " SELECT T0.[OB03], T0.[OB04], T0.[OB05], T0.[OB11], T0.[OB14], T1.[MnfDate], T0.[OB08] "
                sql += "   FROM [@ksOBList] AS T0 INNER JOIN [OBTN] AS T1 ON T0.[OB03] = T1.[DistNumber] "
                sql += "  WHERE EXISTS( SELECT S1.[OB03] FROM [@ksOBList] S1 WHERE S1.[OB07] = @TypeSelect AND S1.[OB08] = @DocEntry AND S1.[OB09] = @LineNum AND S1.[OB10] = '3' AND T0.[OB03] = S1.[OB03]) "
                sql += "  AND T0.[OB08] = @DocEntry  ORDER BY T0.[OB04] "
    ...
    Private Sub SyncToSAPDeliveryNotes()
        ...
        For i As Integer = 0 To dgvSourceList.RowCount - 1
            ...
            sql2 = "  SELECT DISTINCT * FROM [@ksOBList] "
            sql2 += "  WHERE [OB07] = '" & TypeSelect & "' AND "
            sql2 += "        [OB08] = '" & dgvSourceList.Rows(i).Cells("DocEntry").Value & "' AND "
            sql2 += "        [OB09] = '" & dgvSourceList.Rows(i).Cells("LineNum").Value & "'  AND "
            sql2 += "        [OB10] = '3' "
    ...
    Private Sub 刪除完單條碼()
        ...
        SQLCmd.CommandText += "SET @DocEntry   = '" & Label12.Text & "' "
