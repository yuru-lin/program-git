...
Public Class 出庫作業V001bak20180501   '20140917
    ...
    Private Sub T2查詢單據明細()    '品項資料
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                '先寫暫存 Outtmp1
                'SQLQuery += "       SELECT S1.[分時領料單] AS '草稿單號',(ROW_NUMBER() OVER (PARTITION BY S1.[分時領料單] ORDER BY S1.[ID] ASC)) AS '列號', "
                'SQLQuery += "              [存編],[品名],KaiShing.dbo.getRoundN(S1.[派工數量],4) AS '預出數量','銷售' AS '銷售', "
                ...
                'SQLQuery += "         FROM [KaiShingPlug].[dbo].[倉_分時領料] S1 LEFT JOIN [OITM] S2 ON S1.[存編] = S2.[ItemCode] "
                ''SQLQuery += "        WHERE [啟用否] = 'Y' " 
                'SQLQuery += "        WHERE S1.[分時領料單] = '" & 草稿T2.Text & "' AND [啟用否] = 'Y' "
                'SQLQuery += "     ORDER BY [ID] "

                'SQLQuery += " EXEC [dbo].[預_分時領料明細01v] 'ADB',N'" & 草稿T2.Text & "',N'',NULL,NULL "

                SQLQuery += " CREATE TABLE [" & Outtmp1 & "] (  "
                SQLQuery += " 草稿單號 VARCHAR(50) ,列號 INT,存編 VARCHAR(50),品名 VARCHAR(100),預出數量 NUMERIC(20,4),銷售 VARCHAR(6),管理 NVARCHAR(2)) "
                SQLQuery += " INSERT INTO [" & Outtmp1 & "] EXEC [dbo].[預_分時領料明細01v] 'ADB',N'" & 草稿T2.Text & "',N'',NULL,NULL "
                'SQLQuery += " SELECT * FROM [" & Outtmp1 & "] "
                ...
                'SQLQuery += "          dbo.getRoundN((ISNULL((CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE ISNULL(S2.[數量],0) END),0))- S1.[預出數量],4) AS '差異數', dbo.getRoundN(ISNULL(S2.[重量],0),2) AS '出庫重量', S1.[銷售], S1.[管理], 0.0 AS '含稅單價' "
                SQLQuery += "   SELECT S1.[草稿單號], S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                ...
                SQLQuery += "          '' AS '單位2數量', '' AS '單位1KG', '' AS '小單位', '' AS '計價單位', '' AS '銷售金額', '' AS '付款方法', '' AS '員工', '' AS '入雛日', '' AS '生產否', '' AS 'BaseType', '' AS 'BaseEntry', '' AS 'BaseLine', 0 AS '實際金額',"
    ...
    Private Sub T1DGV2_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles T1DGV2.CellClick
        ...
        Select Case 移除作業T1.Text
            Case "24.分時領料" : 查詢別號 = " =   '24'" : 查詢編號 = "0"       '領料  24,25
    ...
    Private Sub SentToPrinter(ByVal olabel As LabeData)
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                Call barcode(T1.Text, T2.Text, "128", T3.Text, "1", "0", "3", "2", olabel.Barcode) '列印條碼 x,y,條碼類型,高度,是否列印條碼碼文,條碼旋轉角度,條碼窄,條碼寬, 容 以點(point)表示
            Case Else
