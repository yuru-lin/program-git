...
Public Class 出庫作業V001   '20140917
    ...
    Private Sub T2查詢單據條碼()
        ...
        Select Case 實際編號
            'Case "0"    '0.排程領料
            Case "0", "1", "2", "3", "5"    '1.單據領料, 2.銷售出庫, 3.存貨領用, 4.寄庫出庫
    ...
    Private Sub T2查詢單據明細()    '品項資料
        ...
        Select Case 作業編號
            Case "0"    '0.排程領料
                '先寫暫存 Outtmp1
                'SQLQuery += " EXEC [dbo].[預_分時領料明細01v] 'ADB',N'" & 草稿T2.Text & "',N'',NULL,NULL "
                同意載入()
                ...
                Else
                    ...
                    'SQLQuery += " SELECT S0.[草稿單號],(ROW_NUMBER() OVER (PARTITION BY S0.[草稿單號] ORDER BY S0.[存編] ASC)) AS '列號',S0.[存編],S0.[品名],S0.[預出數量],S0.[銷售],S0.[管理] "
                    'SQLQuery += " INSERT INTO [" & Outtmp1 & "] "
                    'SQLQuery += "   FROM ( "
                    'SQLQuery += " SELECT '" & 草稿T2.Text & "' AS '草稿單號', S1.[存編], S1.[品名], SUM(CAST(S1.[數量] AS NUMERIC(15,4))) AS '預出數量','銷售' AS '銷售', "
                    ...
                    'SQLQuery += "   FROM " & Outtmp4 & " S1 LEFT JOIN [OITM] S2 ON S1.[存編] = S2.[ItemCode] "
                    'SQLQuery += "        ) S0 " & vbCrLf & vbCrLf
                End If
                ...
                SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售] ) S2 ON S1.[存編] COLLATE Chinese_PRC_Ci_AS = S2.[存編] COLLATE Chinese_PRC_Ci_AS AND S1.[銷售] COLLATE Chinese_PRC_Ci_AS = S2.[銷售] COLLATE Chinese_PRC_Ci_AS ORDER BY 2 "
            ...
            Case "1", "2", "5"    '1.單據領料, 2.銷售出庫, 4.寄庫出庫
                ...
                SQLQuery += "          CASE WHEN (S2.[ManSerNum] = 'Y' AND S2.[ManBtchNum] = 'N') THEN 'S' ELSE "
                SQLQuery += "          CASE WHEN (S2.[ManSerNum] = 'N' AND S2.[ManBtchNum] = 'Y') THEN 'B' ELSE 'X' END END END) AS '管理' "
                SQLQuery += "          ,S1.[PriceAfVAT] AS '含稅單價' "
                ...
                Select Case 作業編號
                    Case "1"    '1.單據領料
                        SQLQuery += "   SELECT CAST(S1.[草稿單號] AS VARCHAR(50)) AS '草稿單號', S1.[列號], S1.[存編], S1.[品名], S1.[預出數量], (CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE dbo.getRoundN(ISNULL(S2.[數量],0),4) END) AS '出庫數量', "
                        SQLQuery += "          dbo.getRoundN((ISNULL((CASE WHEN S1.[管理] = 'F' THEN S1.[預出數量] ELSE ISNULL(S2.[數量],0) END),0))- S1.[預出數量],4) AS '差異數', dbo.getRoundN(ISNULL(S2.[重量],0),2) AS '出庫重量', "
                        SQLQuery += "          S1.[單位2數量], S1.[單位1KG], S1.[小單位], S1.[計價單位], S1.[銷售金額], S1.[付款方法], S1.[員工], S1.[入雛日], S1.[生產否], S1.[BaseType], S1.[BaseEntry], S1.[BaseLine], 0 AS '實際金額', S1.[銷售], S1.[管理], 0.0 AS '含稅單價' "
                        SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN ("
                        SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售] "
                    ...
                    Case "2", "5"    '2.銷售出庫, 4.寄庫出庫
                        ...
                        End Select
                        SQLQuery += "          S1.[銷售], S1.[管理], CAST(S1.[含稅單價] AS NUMERIC(20,4)) AS '含稅單價' "
                        SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN ("
                        SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售] "
            ...
            Case "3"    '3.存貨領用
                '先寫暫存 Outtmp1
                SQLQuery += "    SELECT CAST(S1.[ID] AS VARCHAR(50)) AS '草稿單號', (Row_Number() OVER (partition by ID order by ID Asc, ProCode ASC) - 1) as '列號', S1.[ProCode] AS '存編', S1.[ProName] AS '品名', "
                ...
                SQLQuery += "     FROM " & Outtmp1 & " S1 LEFT  JOIN ("
                SQLQuery += "          SELECT T1.[存編], SUM(CAST(T1.[數量] AS NUMERIC(15,4))) AS '數量', SUM(CAST(T1.[重量] AS NUMERIC(15,2))) AS '重量', T1.[銷售] "
                SQLQuery += "            FROM " & Outtmp4 & " T1 GROUP BY T1.[存編], T1.[銷售] ) S2 ON S1.[存編] COLLATE Chinese_Taiwan_Stroke_CI_AS = S2.[存編] COLLATE Chinese_Taiwan_Stroke_CI_AS AND S1.[銷售] COLLATE Chinese_Taiwan_Stroke_CI_AS = S2.[銷售] COLLATE Chinese_Taiwan_Stroke_CI_AS ORDER BY 2 "
        End Select
        'MsgBox(SQLQuery)
        Dim DBConn As SqlConnection = Login.DBConn
        Dim SQLCmd As SqlCommand = New SqlCommand
    ...
    Private Sub T2查詢單據非單據品項()
        ...
        Dim Outtmp5 As String = ""

        Select Case 作業編號
            Case "0" 'Exit Sub   '0.排程領料
                '    SQLQuery = "  SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS, T0.[品名] COLLATE Chinese_PRC_Ci_AS FROM " & Outtmp4 & " T0 EXCEPT SELECT T1.[存編], T1.[品名] FROM " & Outtmp1 & " T1"

                'SQLQuery = "  SELECT A.[存編], A.[品名], B.[數量]"
                'SQLQuery += " FROM( SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS, T0.[品名] COLLATE Chinese_PRC_Ci_AS FROM " & Outtmp4 & " T0 EXCEPT SELECT T1.[存編], T1.[品名] FROM " & Outtmp1 & " T1) A INNER JOIN "
                'SQLQuery += "     ( SELECT T0.[存編], T0.[品名], SUM(T0.[數量]) AS '數量' FROM " & Outtmp4 & " T0 GROUP BY T0.[存編], T0.[品名]   ) B ON A.[存編] = B.[存編] "
                ''Outtmp5 = Format(Replace(Replace("#SyncOuttmp5" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")
                ''SQLQuery = "   IF (OBJECT_ID('tempdb.." & Outtmp5 & "') IS NOT NULL)  DROP TABLE " & Outtmp5 & " "
                ''SQLQuery += "  SELECT * (SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS, T0.[品名] COLLATE Chinese_PRC_Ci_AS FROM " & Outtmp4 & " T0 EXCEPT SELECT T1.[存編], T1.[品名] FROM " & Outtmp1 & " T1)"
                ''SQLQuery += "  INTO " & Outtmp5 & " "
                ''SQLQuery += "  SELECT * FROM [" & Outtmp5 & "] "
                ''SQLQuery += "  IF (OBJECT_ID('tempdb.." & Outtmp5 & "') IS NOT NULL)  DROP TABLE " & Outtmp5 & " "

                ' ''SQLQuery = " SELECT T0.[存編] COLLATE Chinese_PRC_Ci_AS, T0.[品名] COLLATE Chinese_PRC_Ci_AS FROM " & Outtmp4 & " T0 EXCEPT SELECT T0.[存編], T0.[品名] FROM " & Outtmp1 & " T0 "
