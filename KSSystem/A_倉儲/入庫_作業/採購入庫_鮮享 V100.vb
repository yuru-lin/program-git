...
Public Class 採購入庫_鮮享V100
    ...
    Private Sub 採購入庫_鮮享V100_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Select Case cobSelectType.SelectedIndex
            Case "0"
                sql = "SELECT DISTINCT T0.DocEntry, T0.DocNum, T0.CardCode, T0.CardName, T0.DocDate, T0.TaxDate, T0.DocDueDate, T0.U_attachment, T0.U_CK06, T0.U_Cnum, T0.U_N1, T0.U_N2, T0.U_N3, T0.U_CarDr1, T0.U_CarDr2, T0.U_CarDr3, T0.U_Dlst, T0.U_updnum, T0.U_TpMoney, T0.Comments, T0.U_Dnum FROM ODRF T0  left JOIN DRF1 T1 ON T0.DocEntry = T1.DocEntry " _
                    & "WHERE T0.ObjType='20' AND T0.DocStatus = 'O' AND T0.U_CK06 = '16' AND T0.[CardName] Like '%鮮享%' "
                'sql = "SELECT DISTINCT T0.DocEntry, T0.DocNum, T0.CardCode, T0.CardName, T0.DocDate, T0.TaxDate, T0.DocDueDate, T0.U_attachment, T0.U_CK06, T0.U_Cnum, T0.U_N1, T0.U_N2, T0.U_N3, T0.U_CarDr1, T0.U_CarDr2, T0.U_CarDr3, T0.U_Dlst, T0.U_updnum, T0.U_TpMoney, T0.Comments, T0.U_Dnum FROM ODRF T0  left JOIN DRF1 T1 ON T0.DocEntry = T1.DocEntry " _
                '    & " WHERE T0.ObjType='20' "
            Case "1"
    ...
    Private Sub LoadBarCode()
        Try
            ...
            ElseIf oBorS = "S" Then
                ...
                sql = " SELECT T0.OB02, T0.OB03, T0.OB04, T0.OB05, T0.OB11, T0.OB14, T1.FI109, T1.FI111, T1.FI118 FROM [@ksOBList] AS T0 INNER JOIN [KaiShingPlug].[dbo].[KS_Turn_SS_Barcode] AS T1 ON T0.OB03 = T1.FI106 WHERE "
            ElseIf oBorS = "F" Then
    ...
    Private Sub SyncToSAPPurchaseDeliveryNotes()
        ...
        For i As Integer = 0 To dgvSourceList.RowCount - 1
            ...
            'If IsDBNull(dgvSourceList.Rows(i).Cells("U_p2").Value.ToString) Or Not IsDBNull(dgvSourceList.Rows(i).Cells("U_p2").FormattedValue) = "" Then '重為空白或NULL時
            If IsDBNull(dgvSourceList.Rows(i).Cells("U_p2").Value.ToString) Then '重為空白或NULL時
            Else
                If dgvSourceList.Rows(i).Cells("U_p2").Value.ToString = "" Then
                Else
                    oDelv.Lines.UserFields.Fields.Item("U_p2").Value = dgvSourceList.Rows(i).Cells("U_p2").FormattedValue  '重量
                End If
            End If
            ...
            sql2 = "SELECT DISTINCT * FROM [@ksOBList] AS T0 INNER JOIN [KaiShingPlug].[dbo].[KS_Turn_SS_Barcode] AS T1 ON T0.OB03 = T1.FI106 WHERE T0.OB07='" & TypeSelect & "' and T0.OB08='" & dgvSourceList.Rows(i).Cells("DocEntry").Value & "' and T0.OB09='" & dgvSourceList.Rows(i).Cells("LineNum").Value & "' and T0.OB10='3'"
    ...
    Private Sub SyncSRN(ByVal DocEntry As String)
        ...
        Try
            sql = "UPDATE [OSRN] set U_M1 = T0.OB16, U_M2 = T0.OB04 FROM [@ksOBList] T0 WHERE T0.OB08 = '" & DocEntry & "' and T0.OB03 = [OSRN].DistNumber "
