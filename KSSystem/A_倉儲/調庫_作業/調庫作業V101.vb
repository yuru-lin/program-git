...
Public Class 調庫作業V101   '20141106
    ...
    '暫存資料庫名
    Dim TelySynctmp1 As String = ""
    Dim TelySynctmp2 As String = ""
    Dim TelySynctmp3 As String = ""
    Dim PickingDebitTmp As String = ""
    ...
    Private Sub 調庫作業_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        TelySynctmp1 = Format(Replace(Replace("#SyncPDAtmp1" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")
        TelySynctmp2 = Format(Replace(Replace("#SyncPDAtmp2" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")
        TelySynctmp3 = Format(Replace(Replace("#SyncPDAtmp3" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")
        PickingDebitTmp = Format(Replace(Replace("#PickingDebitTmp" & System.Net.Dns.GetHostName(), "-", ""), " ", ""), "")
    End Sub
    ...
    Private Sub 查詢條碼區別明細()
        ...
        Try
            ...
            SQLCmd.CommandText = " DECLARE @F1 varchar(20) "
            ...
            SQLCmd.CommandText += " GROUP BY T1.[ItemCode], T1.[ItemName], T1.[Quantity], T1.[MnfDate], T0.[F2], T0.[F1], T0.[F4], T1.[U_M1], T1.[U_M2], T1.[U_M8] ,ISNULL(CONVERT(VARCHAR(19),T0.[InDaetTime],120),'') "
    ...
    Private Sub 查詢調整區別SQL_前查()
        ...
        SQLQuery = "     IF (OBJECT_ID('tempdb.." & TelySynctmp1 & "') IS NOT NULL)  DROP TABLE " & TelySynctmp1 & " "
        SQLQuery += "    IF (OBJECT_ID('tempdb.." & TelySynctmp2 & "') IS NOT NULL)  DROP TABLE " & TelySynctmp2 & " "
        SQLQuery += "    SELECT * INTO " & TelySynctmp1 & " FROM [Z_KS_Barcode] TX WHERE TX.[F3] = 'G' AND TX.[F1] IN ('" & 區別sx & "') AND TX.[Del] = 'N' "
        SQLQuery += "    SELECT T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], CAST(SUM(ISNULL(T2.[Quantity],'9999')) AS INT) AS 'Quantity', T1.[U_M1], T1.[U_M8], T1.[MnfDate] "
        SQLQuery += "           INTO " & TelySynctmp2 & " "
        SQLQuery += "      FROM [OSRN] T1 INNER JOIN [OSRQ]               T2 ON T1.[SysNumber] = T2.[SysNumber] AND T1.[AbsEntry] = T2.[MdAbsEntry] AND T1.[ItemCode] = T2.[ItemCode] "
        SQLQuery += "                     INNER JOIN [OITM]               T3 ON T1.[ItemCode]  = T3.[ItemCode] "
        SQLQuery += "                     INNER JOIN " & TelySynctmp1 & " T4 ON T1.[DistNumber] = T4.[F2] "
        SQLQuery += "  GROUP BY T1.[ItemCode], T3.[ItemName], T1.[DistNumber], T1.[U_M2], T1.[U_M1], T1.[U_M8], T1.[MnfDate] "
        SQLQuery += "    SELECT * FROM " & TelySynctmp1 & " "
    ...
    Private Sub 查詢調整區別SQL()
        If DGV3.RowCount > 0 Then : ks1DataSetDGV.Tables("DGV3").Clear() : End If
        ...
        Dim SQLQuery As String = ""
        SQLQuery += "   IF (OBJECT_ID('tempdb.." & TelySynctmp3 & "') IS NOT NULL)  DROP TABLE " & TelySynctmp3 & " "
        SQLQuery += "   SELECT '電宰' AS '類別', T0.[Sid] AS 'ID', T1.[ItemCode] AS '存編', T1.[ItemName] AS '品名', T0.[F2] AS '條碼', T0.[F1] AS '區別', T0.[F4] AS '調至儲位', T1.[U_M2] AS '原始儲位', "
        SQLQuery += "         (CASE WHEN COUNT(ISNULL(T0.[F2],0)) > 1                            THEN '重覆'   ELSE "
        SQLQuery += "          CASE WHEN CAST(SUM(ISNULL(T1.[Quantity],'9999')) AS INT) = '9999' THEN '未入庫' ELSE "
        SQLQuery += "          CASE WHEN T1.[Quantity]  = 0                                      THEN '出庫'   ELSE "
        SQLQuery += "          CASE WHEN T1.[Quantity] >= 1                                      THEN '庫存'   ELSE '不知' END END END END) AS '狀態' "
        SQLQuery += "         ,ISNULL(CONVERT(VARCHAR(19),T0.[InDaetTime],120),'') AS '時間' "
        SQLQuery += "          INTO " & TelySynctmp3 & " "
        SQLQuery += "     FROM " & TelySynctmp1 & " T0 LEFT JOIN "
        SQLQuery += "         (SELECT * FROM " & TelySynctmp2 & ") T1 ON T0.[F2] = T1.[DistNumber] "
        SQLQuery += " GROUP BY T0.[Sid], T1.[ItemCode], T1.[ItemName], T1.[Quantity], T1.[MnfDate], T0.[F2], T0.[F1], T0.[F4], T1.[U_M1], T1.[U_M2], T1.[U_M8], ISNULL(CONVERT(VARCHAR(19),T0.[InDaetTime],120),'') "
        SQLQuery += " ORDER BY T0.[F1], T0.[F2], T1.[ItemCode], T0.[F4] "
        SQLQuery += "   SELECT * FROM " & TelySynctmp3 & " "
    ...
    Private Sub 查詢調整區別MU2UP()
        ...
        Try
            SQLCmd.Connection = DBConn
            SQLCmd.CommandText = "  UPDATE [Z_KS_Barcode]       SET [F6] = ISNULL(T2.[原始儲位],''), [F5] = '1' FROM [Z_KS_Barcode]       T1 INNER JOIN " & TelySynctmp3 & " T2 ON T1.[Sid] = T2.[ID] "
            SQLCmd.CommandText += " UPDATE " & TelySynctmp1 & " SET [F6] = ISNULL(T2.[原始儲位],''), [F5] = '1' FROM " & TelySynctmp1 & " T1 INNER JOIN " & TelySynctmp3 & " T2 ON T1.[Sid] = T2.[ID] "
            SQLCmd.CommandTimeout = 100000
            SQLCmd.Parameters.Clear()
            SQLCmd.ExecuteNonQuery()
        Catch ex As Exception
    ...
    Private Sub ChkLoc()
        If DGV5.RowCount > 0 Then : ks1DataSetDGV.Tables("DGV5").Clear() : End If
        If 查詢錯誤 = False Then : Exit Sub : End If
        ...
        Dim SQLQuery As String = ""
        SQLQuery = "    IF (OBJECT_ID('tempdb.." & PickingDebitTmp & "') IS NOT NULL)  DROP TABLE " & PickingDebitTmp & " "
        SQLQuery += "   SELECT T0.[U_M2] AS 'F4', COUNT(T0.[U_M2]) AS 'T2Q' INTO " & PickingDebitTmp & " "
        SQLQuery += "     FROM [OSRN] T0 INNER JOIN [OSRQ] T1 ON T0.[SysNumber] = T1.[SysNumber] AND T0.[AbsEntry] = T1.[MdAbsEntry] AND T0.[ItemCode] = T1.[ItemCode] "
        SQLQuery += "    WHERE T0.[U_M2] IN ('" & 儲位sx & "') AND ISNULL(T1.[Quantity],0) > 0 "
        SQLQuery += " GROUP BY T0.[U_M2] "
        SQLQuery += "   SELECT T0.[F4] AS '儲位', ISNULL(T0.[T0Q],0) AS '調整數量', ISNULL(T1.[T1Q],0) AS '重覆數量', ISNULL(T2.[T2Q],0) AS '目前數量', "
        SQLQuery += "          ISNULL(T0.[T0Q],0) + ISNULL(T2.[T2Q],0) - ISNULL(T1.[T1Q],0) AS '調後數量', ISNULL(T3.[U_Pmaxnum],0) AS '最大數', "
        SQLQuery += "         (ISNULL(T0.[T0Q],0) + ISNULL(T2.[T2Q],0) - ISNULL(T1.[T1Q],0)) - ISNULL(T3.[U_Pmaxnum],0) AS '超出數' "
        SQLQuery += "     FROM (SELECT DISTINCT T0.[F4] AS 'F4', COUNT(T0.[F2]) AS 'T0Q' FROM " & TelySynctmp1 & " T0 GROUP BY T0.[F4]) T0 LEFT JOIN "
        SQLQuery += "          (SELECT DISTINCT T0.[F4] AS 'F4', COUNT(T0.[F6]) AS 'T1Q' FROM " & TelySynctmp1 & " T0 WHERE T0.[F5] = '1' AND T0.[F4] = T0.[F6] "
        SQLQuery += "         GROUP BY T0.[F4] ) T1 ON T0.[F4] = T1.[F4] LEFT JOIN "
        SQLQuery += "          (SELECT * FROM " & PickingDebitTmp & ") T2 ON T0.[F4] = T2.[F4] LEFT JOIN [@WPLACE] T3 ON T0.[F4] = T3.[U_Pcode] "
    ...
    Private Sub 查詢客編T2DGV2()
        ...
        SQLCmd.CommandText = "  SELECT ISNULL([F1],'') AS '區別', '' AS '調至庫位' "
        SQLCmd.CommandText += "   FROM [Z_KS_Barcode] "
    ...
    Private Sub DoingSQL()
        ...
        Try
            ...
            'SQLCmd.CommandText = "   UPDATE [OSRN] SET [U_M2] = T1.[F4] FROM [OSRN] T0 INNER JOIN #TelySynctmp1 T1 ON T0.[DistNumber] = T1.[F2] "
            'SQLCmd.Parameters.Clear()
            'SQLCmd.ExecuteNonQuery()

            SQLCmd.CommandText = "  UPDATE [OSRN] SET [U_M2] = T1.[F4] FROM [OSRN] T0 INNER JOIN " & TelySynctmp1 & " T1 ON T0.[DistNumber] = T1.[F2] "
            SQLCmd.CommandText += " INSERT INTO [@ChgLocMgn] ([DocNum],[SRNType],[Part],[FromLoc],[DistNumber],[ItemCode],[ItemName],[ToLoc]) "
            SQLCmd.CommandText += "      SELECT @ADate, '1' AS 'F5', 區別, [原始儲位], [條碼], [存編], [品名], [調至儲位] FROM " & TelySynctmp3 & " "
            SQLCmd.Parameters.Clear()
        ...
        SQLCmd.Connection = DBConn
        SQLCmd.CommandText = " DELETE [Z_KS_Barcode] FROM [Z_KS_Barcode] T0 INNER JOIN " & TelySynctmp1 & " T1 ON T0.[Sid] = T1.[Sid] "
        SQLCmd.ExecuteNonQuery()
        查詢條碼區別()
    ...
    Private Sub 指定調庫更新()
        ...
        For i = 0 To DGV1.RowCount - 1
            If Microsoft.VisualBasic.Left(DGV1.Rows(i).Cells("D4區別").Value, 1) = "G" Then
                SQLADD += " UPDATE [KaiShingPlug].[dbo].[倉_指定入庫Main] SET [啟用] = 'U',[修改時間] = GETDATE() FROM [KaiShingPlug].[dbo].[倉_指定入庫Main] WHERE [單號] = '" & DGV1.Rows(i).Cells("D4區別").Value & "' AND [啟用] = 'Y' " & vbCrLf
                SQLADD += " UPDATE [KaiShingPlug].[dbo].[倉_指定入庫List] SET [啟用] = 'U',[修改時間] = GETDATE() FROM [KaiShingPlug].[dbo].[倉_指定入庫List] WHERE [單號] = '" & DGV1.Rows(i).Cells("D4區別").Value & "' AND [啟用] = 'Y' " & vbCrLf
            End If
        Next

        'MsgBox(SQLADD)
        If SQLADD = "" Then : Exit Sub : End If
        Try
    ...
    Private Sub 查詢調整區別錯誤條碼SQL()
        ...
        'Dim sss As String
        Dim SQLQuery As String = ""
        SQLQuery = "    SELECT '電宰' AS '類別', T1.[ItemCode] AS '存編', T1.[ItemName] AS '品名', T0.[F2] AS '條碼', T0.[F1] AS '區別', T0.[F4] AS '調至儲位', T1.[U_M2] AS '原始儲位', "
        SQLQuery += "           (CASE WHEN COUNT(ISNULL(T0.[F2],0)) > 1 THEN '重覆' ELSE CASE WHEN CAST(SUM(ISNULL(T1.[Quantity],'9999')) AS INT) = '9999' THEN '未入庫' ELSE CASE WHEN T1.[Quantity] = 0 THEN '出庫' ELSE CASE  WHEN T1.[Quantity] >= 1 THEN '庫存' ELSE '不知' END END END END) AS '狀態' "
        SQLQuery += "          ,ISNULL(CONVERT(VARCHAR(19),T0.[InDaetTime],120),'') AS '時間' "
        SQLQuery += "     FROM " & TelySynctmp1 & " T0 LEFT JOIN "
        SQLQuery += "         (SELECT * FROM " & TelySynctmp2 & ") T1 ON T0.[F2] = T1.[DistNumber] "
        SQLQuery += " GROUP BY T1.[ItemCode], T1.[ItemName], T1.[Quantity], T1.[MnfDate], T0.[F2], T0.[F1], T0.[F4], T1.[U_M1], T1.[U_M2], T1.[U_M8], ISNULL(CONVERT(VARCHAR(19),T0.[InDaetTime],120),'') "
        SQLQuery += "   HAVING CAST(SUM(ISNULL(T1.[Quantity],'9999')) AS INT) <> 1  "
        SQLQuery += " ORDER BY T0.[F1], T0.[F2], T1.[ItemCode], T0.[F4] "
